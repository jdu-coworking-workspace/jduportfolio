...
import { 
    checkprofileBackPageAtom, 
    checkprofileSortByAtom, 
    checkprofileSortOrderAtom, 
    listReturnPathAtom, 
    studentsBackPageAtom, 
    studentsSortByAtom, 
    studentsSortOrderAtom, 
    tableScrollPositionAtom 
} from '../../../atoms/store'
...
const StudentProfile = ({ userId = 0, isPublic = false }) => {
    ...
    // UUID va studentId ni URL'dan olamiz
    ...
    // Tizimga kirgan talaba ID'sini olish
    ...
    // ID aniqlash iyerarxiyasi
    let id
    if (isPublic && uuid) {
        id = uuid // Public link uchun UUID ustuvor
    ...
    useEffect(() => {
        // Public bo'lsa initialization (auth) kutish shart emas
    ...
    // Bookmark holatini faqat Recruiter uchun tekshiramiz
    useEffect(() => {
    ...
    if (loading) return <Box sx={{ display: 'flex', justifyContent: 'center', p: 5 }}>{t('loading_student_profile')}</Box>

    if (error) {
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', p: 5, color: 'red' }}>
                <div>{t('error_label')}: {translations[language][error] ? t(error) : error}</div>
        ...
        <Box sx={{ borderRadius: '10px' }}>
            ...
            {!isPublic && role !== 'Student' && (
                <Box className={styles.topControlButtons}>
                    ...
                    <Button 
                        disabled={visibleRowsStudentIds.length <= 0 || visibleRowsStudentIds.findIndex(ids => ids.isCurrent) + step >= visibleRowsStudentIds.length} 
                    ...
                    <Box className={styles.nameEmailContainer}>
                        ...
                        <Box>
                            ...
                            <Box className={styles.statusChipContainer}>
                                {student.visibility ? <div style={{ color: '#7ED6A7' }}>{t('published')}</div> : <div style={{ color: '#812958' }}>{t('private')}</div>}
                            </Box>
                        </Box>
                    </Box>
                </Box>
