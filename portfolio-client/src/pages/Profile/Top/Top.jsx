									...
									{[
										{
											label: 'JLPT:',
											getValue: () => {
												const v = editData.draft.jlpt ? getJLPTData(editData.draft.jlpt).highest : getJLPTData(student.jlpt).highest
										...
										},
										{
											...
											getValue: () => {
												const v = editData.draft.jdu_japanese_certification ? getJLPTData(editData.draft.jdu_japanese_certification).highest : getJLPTData(student.jdu_japanese_certification).highest
												return v === '未提出' || !v ? t('not_submitted') : v
											},
										},
										{
											label: `${t('japaneseSpeechContest')}:`,
											getValue: () => {
												const v = editData.draft.japanese_speech_contest ? getCertificateData(editData.draft.japanese_speech_contest).highest : getCertificateData(student.japanese_speech_contest).highest
												return v === '未提出' || !v ? t('not_submitted') : v
											},
											suffix: t('rank'),
										},
										{
											label: `${t('itContest')}:`,
											getValue: () => {
												const v = editData.draft.it_contest ? getCertificateData(editData.draft.it_contest).highest : getCertificateData(student.it_contest).highest
												return v === '未提出' || !v ? t('not_submitted') : v
											},
											suffix: t('rank'),
										},
										...
										<div key={i} style={{ display: 'flex', alignItems: 'center', height: 36 }}>
											...
											<span style={{ padding: '6px 20px', fontWeight: 500, fontSize: 14, border: '1px solid #e0e0e0', borderRadius: 6, background: editMode ? '#f5f5f5' : '#fff', color: editMode ? '#666' : '#000', cursor: editMode ? 'not-allowed' : 'default' }} title={editMode ? 'この情報はKintoneから管理されています' : ''}>
												{item.getValue()}
											</span>
											{item.suffix && <span style={{ marginLeft: 8 }}>{item.suffix}</span>}
										</div>
									))}
								</div>
							</div>
