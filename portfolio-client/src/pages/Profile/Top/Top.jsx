	...
	return (
		<>
			...
			<Box mb={2}>
				...
				<Dialog open={showCvDownloadDialog} onClose={() => setShowCvDownloadDialog(false)} maxWidth='md' fullWidth>
					<DialogTitle sx={{ fontWeight: 'bold', color: '#e65100', display: 'flex', alignItems: 'center', gap: 1, fontSize: '1.2rem' }}>{language === 'ja' ? '⚠️ 履歴書をダウンロードする前に必ずお読みください！' : language === 'en' ? '⚠️ Please read carefully before downloading your CV!' : language === 'ru' ? '⚠️ Обязательно прочтите перед скачиванием резюме!' : "⚠️ Rezyumeni yuklab olishdan avval o'qing!"}</DialogTitle>
					<DialogContent dividers sx={{ maxHeight: '60vh' }}>
						{/* Main warning text */}
						<DialogContentText sx={{ mb: 2, lineHeight: 1.8 }}>{language === 'ja' ? '学生の皆様へ この履歴書（CV）は、ウェブサイトのポートフォリオの情報をもとにシステムが自動で作成したものです。そのため、内容が100％正しいとは限りません。ダウンロードした後は、必ず「見本 履歴書」と見比べて、自分で内容をチェックしてください！' : language === 'en' ? 'Dear Students, This CV is automatically generated by the system based on the information in your website portfolio. Therefore, the content may not be 100% accurate. After downloading, please be sure to compare it with the "見本 履歴書" (Sample CV) and manually check the content yourself!' : language === 'ru' ? 'Уважаемые студенты! Это резюме (CV) автоматически генерируется системой на основе информации из вашего портфолио на сайте. Поэтому его содержимое может быть не на 100% точным. После скачивания обязательно сравните его с "見本 履歴書" (Образец резюме) и проверьте данные вручную!' : "Hurmatli talaba, Ushbu rezyume (CV) veb-saytdagi portfoliongiz asosida tizim tomonidan avtomatik shakllantirilgan bo'lib, uning ichidagi ma'lumotlar 100% aniq yoki yakuniy bo'lmasligi mumkin. Yuklab olganingizdan so'ng, uni \"見本 履歴書\" (Namuna) bilan solishtirib, qo'lda tekshirib chiqishingiz qat'iyan talab qilinadi!"}</DialogContentText>

						{/* Point 1: 職務経歴書 */}
						<Box sx={{ backgroundColor: '#fff3e0', border: '1px solid #ff9800', borderRadius: '8px', p: 2, mb: 2 }}>
							<Box sx={{ fontWeight: 'bold', mb: 1, color: '#e65100', fontSize: '1rem' }}>{language === 'ja' ? '❗️ 1) 「職務経歴書」について' : language === 'en' ? '❗️ 1) About "職務経歴書" (Work History)' : language === 'ru' ? '❗️ 1) Раздел "職務経歴書" (Опыт работы)' : '❗️ 1) 「職務経歴書」 (Ish tajribasi)'}</Box>
							<DialogContentText sx={{ fontSize: '0.9rem', lineHeight: 1.7 }}>{language === 'ja' ? '最初の表の部分には、大学の「Cowork（コワーク）」で参加したプロジェクトだけを書きます。自動でデータが入力されるため、他の情報と混ざっている可能性があります。お願いですから、「Cowork」のプロジェクトと、外での経験（アルバイト、フリーランス、他の会社のプロジェクトなど）はしっかり分けて、それぞれ正しい場所に書かれているか確認してください。' : language === 'en' ? 'In the first table section, only include projects you participated in through the university\'s "Cowork" system. Because the data is generated automatically, it might be mixed with other information. Please ensure that "Cowork" projects are strictly separated from outside experiences (part-time jobs, freelance work, other companies\' projects, etc.) and that each is written in its correct section.' : language === 'ru' ? 'В первую таблицу вносятся только те проекты, в которых вы участвовали через университетскую систему "Cowork". Поскольку данные загружаются автоматически, они могут смешаться с другой информацией. Пожалуйста, строго разделяйте проекты "Cowork" и внешний опыт (подработки, фриланс, проекты других компаний) и проверьте, чтобы всё было записано в соответствующих разделах.' : 'Ushbu qismga faqat universitetning "Cowork" tizimida ishlagan loyihalaringiz kiritilishi kerak. Ma\'lumotlar avtomatik yuklanayotgani sababli aralashib qolish ehtimoli bor. Iltimos, "Cowork" va tashqaridagi loyihalarni (arubaito, frilanserlik, boshqa kompaniya loyihalari) alohida ajratib, har birini o\'zining tegishli joyiga yozganingizni tekshirib oling.'}</DialogContentText>
						</Box>

						{/* Point 2: Addresses */}
						<Box sx={{ backgroundColor: '#e3f2fd', border: '1px solid #2196f3', borderRadius: '8px', p: 2, mb: 2 }}>
							<Box sx={{ fontWeight: 'bold', mb: 1, color: '#1565c0', fontSize: '1rem' }}>{language === 'ja' ? '❗️ 2) 住所 – 「現住所」と「連絡先」' : language === 'en' ? '❗️ 2) Addresses – "現住所" and "連絡先"' : language === 'ru' ? '❗️ 2) Адреса – "現住所" и "連絡先"' : '❗️ 2) Manzillar – 「現住所」 va 「連絡先」'}</Box>
							<DialogContentText sx={{ fontSize: '0.9rem', lineHeight: 1.7 }}>
								{language === 'ja' ? (
									<>
										<strong>「現住所」</strong>: 今、あなたが実際に住んでいる場所の住所を書きます。
										<br />
										<strong>「連絡先」</strong>: あなたの保護者（ご両親など）が住んでいる住所を書きます。
										<br />
										もし両方が同じ場所の場合は、下の部分（連絡先）には<strong>「同上」</strong>とだけ書いてください。正しく入力されているか確認してください。
									</>
								) : language === 'en' ? (
									<>
										<strong>"現住所" (Current Address)</strong>: Write the address where you currently live.
										<br />
										<strong>"連絡先" (Contact Address)</strong>: Write the address where your guardians (parents, etc.) live.
										<br />
										If both addresses are the same, simply write <strong>"同上"</strong> (Same as above) in the bottom section (Contact Address). Please check that this is entered correctly.
									</>
								) : language === 'ru' ? (
									<>
										<strong>「現住所」 (Текущий адрес)</strong>: Здесь указывается адрес, по которому вы проживаете в данный момент.
										<br />
										<strong>「連絡先」 (Контактный адрес)</strong>: Здесь указывается адрес проживания ваших опекунов (родителей).
										<br />
										Если оба адреса совпадают, просто напишите <strong>「同上」</strong> (То же, что и выше) в нижнем разделе (Контактный адрес). Пожалуйста, проверьте правильность заполнения.
									</>
								) : (
									<>
										<strong>「現住所」</strong>: Bu sizning hozirgi vaqtda yashab turgan manzilingiz kiritiladigan joy.
										<br />
										<strong>「連絡先」</strong>: Bu esa vasiylaringiz (ota-onangiz) istiqomat qilayotgan manzil.
										<br />
										Agar ikkala manzil ham bir xil bo&apos;lsa, pastki qismiga (連絡先&apos;ga) shunchaki <strong>「同上」</strong> deb yozishingiz kerak. Iltimos, to&apos;g&apos;ri to&apos;ldirilganligini tekshirib oling.
									</>
								)}
							</DialogContentText>
						</Box>
						...
						{/* Point 3: 自己PR */}
						<Box sx={{ backgroundColor: '#fce4ec', border: '1px solid #e91e63', borderRadius: '8px', p: 2, mb: 2 }}>
							<Box sx={{ fontWeight: 'bold', mb: 1, color: '#c62828', fontSize: '1rem' }}>{language === 'ja' ? '❗️ 3) 「自己PR」' : language === 'en' ? '❗️ 3) "自己PR" (Self PR)' : language === 'ru' ? '❗️ 3) Раздел "自己PR" (О себе)' : "❗️ 3) 「自己PR」 (O'zini tanishtirish)"}</Box>
							<DialogContentText sx={{ fontSize: '0.9rem', lineHeight: 1.7 }}>{language === 'ja' ? 'この部分は、自動で作られたテンプレートのままにしてはいけません。提出する会社や状況に合わせて、毎回自分で書き直す必要があります。自分の言葉で正しく修正されているか確認してください。' : language === 'en' ? 'You must not leave this section as the auto-generated template. You need to rewrite it yourself every time, adapting it to the company or situation you are applying for. Please check that it has been properly customized in your own words.' : language === 'ru' ? 'Этот раздел нельзя оставлять в виде автоматически сгенерированного шаблона. Вам нужно каждый раз переписывать его самостоятельно, адаптируя под конкретную компанию или ситуацию. Убедитесь, что текст исправлен и написан вашими собственными словами.' : "Ushbu qismni tayyor andoza bilan qoldirib bo'lmaydi. Talaba har doim bu qismni qayerga (qaysi kompaniyaga yoki vaziyatga) topshirayotganiga qarab, moslab qaytadan yozishi talab etiladi. O'zingizga moslab tahrirlanganini tekshirib oling."}</DialogContentText>
						</Box>

						{/* 職務経歴書 skills section */}
						<Box sx={{ backgroundColor: '#fff3e0', border: '1px solid #ff9800', borderRadius: '8px', p: 2, mb: 2 }}>
							<Box sx={{ fontWeight: 'bold', mb: 1 }}>{language === 'ja' ? '■ 職務経歴書（2枚目）' : language === 'en' ? '■ Work History - 2nd sheet (職務経歴書)' : language === 'ru' ? '■ Опыт работы — 2-й лист (職務経歴書)' : '■ Ish tajribasi — 2-varaq (職務経歴書)'}</Box>
							<Box component='ul' sx={{ m: 0, pl: 3 }}>
								<li>{language === 'ja' ? 'フロントエンド　○○○' : language === 'en' ? 'Frontend　○○○' : language === 'ru' ? 'Фронтенд　○○○' : 'Frontend　○○○'}</li>
								<li>{language === 'ja' ? 'バックエンド　○○○' : language === 'en' ? 'Backend　○○○' : language === 'ru' ? 'Бэкенд　○○○' : 'Backend　○○○'}</li>
								<li>{language === 'ja' ? 'フレームワーク　○○○' : language === 'en' ? 'Framework　○○○' : language === 'ru' ? 'Фреймворк　○○○' : 'Framework　○○○'}</li>
								<li>{language === 'ja' ? '他　○○○' : language === 'en' ? 'Other　○○○' : language === 'ru' ? 'Другое　○○○' : 'Boshqa　○○○'}</li>
							</Box>
						</Box>

						{/* 言語スキル section */}
						<Box sx={{ backgroundColor: '#e3f2fd', border: '1px solid #2196f3', borderRadius: '8px', p: 2, mb: 2 }}>
							<Box sx={{ fontWeight: 'bold', mb: 1 }}>{language === 'ja' ? '■ 言語スキル' : language === 'en' ? '■ Language Skills (言語スキル)' : language === 'ru' ? '■ Языковые навыки (言語スキル)' : "■ Til ko'nikmalari (言語スキル)"}</Box>
							<Box component='ul' sx={{ m: 0, pl: 3 }}>
								<li>{language === 'ja' ? '・言語スキル）　レベル' : language === 'en' ? '・Language Skill)　Level' : language === 'ru' ? '・Языковой навык)　Уровень' : "・Til ko'nikmasi)　Daraja"}</li>
								<li>{language === 'ja' ? '英語　IELTS ○ / ロシア語　○○ / 日本語　○○' : language === 'en' ? 'English　IELTS ○ / Russian　○○ / Japanese　○○' : language === 'ru' ? 'Английский　IELTS ○ / Русский　○○ / Японский　○○' : 'Ingliz tili　IELTS ○ / Rus tili　○○ / Yapon tili　○○'}</li>
							</Box>
						</Box>

						<DialogContentText sx={{ fontWeight: 'bold', color: '#d32f2f', mb: 1 }}>{language === 'ja' ? '上記の項目にご自身のスキルや情報を記入してください。' : language === 'en' ? 'Please fill in the above sections with your own skills and information.' : language === 'ru' ? 'Пожалуйста, заполните вышеуказанные разделы своими навыками и информацией.' : "Iltimos, yuqoridagi qismlarga o'z ko'nikmalaringiz va ma'lumotlaringizni kiriting."}</DialogContentText>

						{/* Help tip */}
						<Box sx={{ backgroundColor: '#e8f5e9', border: '1px solid #4caf50', borderRadius: '8px', p: 2 }}>
							<DialogContentText sx={{ fontSize: '0.9rem', color: '#2e7d32' }}>{language === 'ja' ? '💡 もし履歴書の修正で分からないことや問題があれば、遠慮なく先輩や大学のスタッフ（職員）に相談してください。' : language === 'en' ? '💡 If you have any questions or problems editing your CV, please do not hesitate to ask your senpais or university staff for help.' : language === 'ru' ? '💡 Если у вас возникнут вопросы или трудности при редактировании резюме, не стесняйтесь обращаться за помощью к семпаям или сотрудникам (staff) университета.' : "💡 Agar rezyumeni tahrirlashda tushunmovchiliklar yoki qandaydir muammolar paydo bo'lsa, iltimos, senpaylar yoki universitet xodimlariga (staff) murojaat qiling."}</DialogContentText>
						</Box>
					</DialogContent>
					<DialogActions>
						...
						<Button
							...
							color='primary'
						>
							{language === 'ja' ? '確認してダウンロード' : language === 'en' ? 'Confirm & Download' : language === 'ru' ? 'Подтвердить и скачать' : 'Tasdiqlash va yuklab olish'}
						</Button>
					</DialogActions>
				</Dialog>
			</Box>
