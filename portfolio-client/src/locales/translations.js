const translations = {
	en: {
		// Recruiter profile new keys
		age: 'Age',
		tagline: 'Tagline',
		company_information: 'Company Information',
		company_name: 'Company Name',
		company_Address: 'Address',
		company_website: 'Website',
		company_description: 'Company Overview',
		company_overview: 'Company Overview',
		business_overview: 'Business Content',
		employee_Count: 'Employees',
		established_Date: 'Established',
		company_capital: 'Capital',
		company_revenue: 'Revenue',
		company_representative: 'Representative',
		job_details: 'Recruitment Overview',
		job_title: 'Recruiting Position',
		job_description: 'Job Description',
		japanese_level: 'Japanese Level',
		application_requirements_other: 'Other Application Requirements',
		required_skills: 'Required: Experience/Skills',
		welcome_skills: 'Welcome: Experience/Skills',
		recommended_skills: 'Recommended: Experience/Skills',
		recommended_licenses: 'Recommended: Licenses/Certificates',
		recommended_other: 'Recommended: Other',
		number_of_openings: 'Number of Openings',
		employment_type: 'Employment Type',
		probation_period: 'Probation Period',
		employment_period: 'Employment Period',
		work_location: 'Work Location',

		compensation_benefits: 'Compensation & Benefits',
		salary: 'Salary',
		salary_increase: 'Salary Increase',
		bonus: 'Bonus',
		allowances: 'Other Allowances (Welfare)',
		holidays_vacation: 'Holidays/Vacation',
		benefits: 'Social Insurance',

		other: 'Other',
		other_notes: 'Other',

		selection_info: 'Selection Process',
		selection_process: 'Hiring Process',
		interview_method: 'Interview Method',
		// Intro page section
		intro_page: 'Introduction Page',
		intro_page_thumbnail: 'Intro Page Thumbnail',
		intro_page_links: 'Intro Page Links',
		newIntroLinkTitle: 'Link Title',
		newIntroLinkUrl: 'Link URL',
		intro_link_title: 'Link Title',
		max_four_links: 'You can add up to 4 links',
		// HR options
		retirement_benefit: 'Retirement benefit',
		telework_availability: 'Telework availability',
		housing_availability: 'Housing availability (dorm/company housing)',
		relocation_support: 'Airfare/relocation expense coverage',
		airport_pickup: 'Airport pickup on arrival',
		rank: 'rank',
		title: 'Title',
		description: 'Description',
		sourseLink: 'Sourse Link',
		hashtag: 'Hashtag',
		CreateNews: 'Create news',
		EditNews: 'Edit news',
		visibleToRecruiter: 'Visible to recruiter',
		newsVisibility: 'News Visibility',
		universityNews: 'University News',
		universityNewsDescription: 'Visible to everyone',
		recruiterNews: 'Recruiter News',
		recruiterNewsDescription: 'Visible to non-recruiters',
		imageRequired: 'Image is required',
		descriptionRequired: 'Description is required',
		fillRequiredFields: 'Please fill in all required fields (Title, Description, and Image)',
		searchByHashtag: 'Search by hashtag, company name, or news title...',
		checkOut: 'Check out the latest announcements and offers from companies',
		newsHighlights: 'News highlights',
		news: 'Company news',
		Qualifications: 'Qualifications',
		studetentSkillManagment: 'Student skill management',
		studentCredits: "Student's number of credits",
		origin: 'origin',
		major: 'Field of study',
		jobType: 'Desired job type',
		qualification: 'Qualification',
		japaneseSpeechContest: 'Japanese Speech Contest',
		itContest: 'IT contest',
		languageSkills: 'Language skills',
		japanese: 'Japanese',
		general: 'General',
		home: 'Home',
		profile: 'Profile',
		student_search: 'Student Search',
		student_check: 'Check Students',
		staff: 'Staff',
		recruiter: 'Recruiter',
		bookmarked: 'Bookmarked',
		settings: 'Settings',
		help: 'Help',
		student_qa: 'Student Questions',
		logout: 'Log Out',
		Are_you_sure: 'Are you sure you want to log out?',
		yesModal: 'Yes',
		noModal: 'No',
		jdu_portfolio: 'JDU Portfolio',
		japan: 'Japan',
		uzbekistan: 'Uzbekistan',
		next_button: 'Go to student search ➜',
		large_class_photo_alt: 'Large class photo',
		group_photo_alt: 'Group photo',
		changes_saved: 'Changes saved successfully!',
		grade: 'Grade',
		grade1: '1st grade',
		grade2: '2nd grade',
		grade3: '3rd grade',
		grade4: '4th grade',
		programming_languages: 'Programming Languages',
		jlpt: 'Japanese Language Proficiency Test',
		ielts: 'IELTS',
		jdu_certification: 'JDU Japanese Certification Test',
		credits: 'Credits',
		credits_unit: ' Credits within',
		partner_university: 'Partner University',
		special_skills: 'Special Skills',
		yes: 'Yes',
		no: 'No',
		tokyo_communication_university: 'Tokyo Communication University',
		kyoto_tachibana_university: 'Kyoto Tachibana University',
		sanno_university: 'Sanno University',
		sanno_junior_college: 'Sanno Junior College (Jiyugaoka)',
		niigata_sangyo_university: 'Niigata Sangyo University',
		otemae_university: 'Otemae University',
		okayama_university_of_science: 'Okayama University of Science',
		unaffiliated: 'Unaffiliated',
		student: 'Student',
		rows_per_page: 'Rows per page:',
		search: 'Search',
		any: 'Any',
		all: 'All',
		search_items: 'Search...',
		select_all_filtered: 'Select All (filtered)',
		clear_filtered: 'Clear (filtered)',
		selected: 'Selected',
		select_all: 'Select All',
		clear_all: 'Clear All',
		reset: 'Reset',
		name_search: 'Search by name',
		department: 'Department',
		position: 'Position',
		phone_number: 'Phone Number',
		name: 'Name',
		edit: 'Edit',
		cancel: 'Cancel',
		save: 'Save',
		sync: 'Sync',
		user_avatar: 'User Avatar',
		company_logo: 'Company Logo',
		upload_picture: 'Upload Picture',
		user: 'User',
		first_name: 'First Name',
		first_name_furigana: 'First Name (Furigana)',
		last_name: 'Last Name',
		last_name_furigana: 'Last Name (Furigana)',
		furigana_help: 'Enter the pronunciation in hiragana/katakana',
		phone: 'Phone',
		email: 'Email',
		change_password: 'Change Password',
		current_password: 'Password',
		new_password: 'New Password',
		confirm_password: 'Confirm Password',
		toggle_password_visibility: 'Toggle Password Visibility',
		contact_info: 'Contact Information',
		contact_email: 'Contact Email',
		contact_phone: 'Contact Phone',
		working_hours: 'Working Hours',
		location: 'Location',
		user_fetch_error: 'Failed to fetch user data.',
		profile_update_success: 'Profile updated successfully.',
		profile_update_failed_retry: 'Failed to update profile. Please try again.',
		unknown_role_error: 'Unknown role encountered.',
		password_mismatch: 'Passwords do not match.',
		current_password_required: 'Please enter your current password.',
		unknown_role: 'Unknown role.',
		fetch_user_failed: 'Failed to fetch user data.',
		profile_updated: 'Profile updated successfully.',
		profile_update_failed: 'Failed to update profile. Please try again.',
		sync_successful: 'Synchronization successful.',
		sync_failed: 'Synchronization failed. Please try again.',
		notFoundTitle: '404 - Page Not Found',
		notFoundMessage: 'The page you are looking for does not exist.',
		backToHome: 'Back to Home',
		maintenanceTitle: 'Under Maintenance',
		checkingMaintenance: 'Checking maintenance status...',
		checkingStatus: 'Automatically checking status...',
		// Maintenance page translations
		maintenance_announcement: 'Maintenance Announcement',
		announcement_settings: 'Announcement Settings',
		message: 'Message',
		enter_announcement_message: 'Enter announcement message...',
		visibility_settings: 'Visibility Settings',
		make_public: 'Make announcement visible to all users on login page',
		announcement_visible_to_all_users: 'This announcement is currently visible to all users on the login page.',
		announcement_hidden_from_users: 'This announcement is currently hidden from users.',
		created_at: 'Created',
		last_updated: 'Last Updated',
		message_required: 'Message is required',
		announcement_updated: 'Announcement updated successfully',
		error_updating_announcement: 'Error updating announcement',
		please_update_announcement_first: 'Please update announcement first',
		announcement_made_public: 'Announcement is now visible to all users',
		announcement_made_private: 'Announcement is now hidden from users',
		error_toggling_status: 'Error toggling status',
		unauthorizedTitle: '403 - Access Denied',
		unauthorizedMessage: 'You do not have permission to access this page.',
		unauthorizedButton: 'Go to Login Page',
		back: 'Back',
		add: 'Add',
		saveDraft: 'New Draft',
		updateDraft: 'Update Draft',
		submitAgree: 'Submit/Consent',
		edit_profile: 'Edit Profile',
		liveProfile: 'Live Profile',
		draftProfile: 'Draft Profile',
		qa_edit: 'Edit QA',
		faq_heading: 'FAQ',
		faq_no_answer: 'No answer yet',
		q_edit: 'Edit Questions',
		age: 'Age',
		student_id: 'Student ID',
		top: 'Top',
		qa: 'Q&A',
		stats: 'Credits and Skills',
		errorFetchingStudent: 'Error fetching student data.',
		changesSavedSuccessfully: 'Changes saved successfully!',
		errorSavingChanges: 'Error saving changes.',
		errorUploadingFile: 'Error uploading file.',
		savingChanges: 'Saving changes...',
		changesSaved: 'Changes saved',
		dataRecovered: 'Data recovered successfully',
		recoverUnsavedChanges: 'Recover Unsaved Changes?',
		unsavedChangesFound: 'We found unsaved changes from your previous editing session. Would you like to restore them?',
		lastModified: 'Last modified',
		discard: 'Discard',
		restore: 'Restore',
		unsavedChangesTitle: 'Unsaved Changes',
		unsavedChangesMessage: 'You have unsaved changes. Are you sure you want to discard them?',
		continueEditing: 'Continue Editing',
		discardChanges: 'Discard Changes',
		unsavedChangesLanguageTitle: 'Save changes before switching language?',
		unsavedChangesLanguageMessage: 'You have unsaved changes. Would you like to save them before changing the language?',
		discardAndSwitch: 'Discard & Switch',
		saveAndSwitch: 'Save & Switch',
		dataRestoredAfterLanguageSwitch: 'Your data has been restored after language switch',
		unsavedChangesNavigationTitle: 'Save changes before leaving?',
		unsavedChangesNavigationMessage: 'You have unsaved changes. Would you like to save them before leaving this page?',
		discardAndLeave: 'Discard & Leave',
		saveAndLeave: 'Save & Leave',
		fileUploadSuccess: 'File uploaded successfully.',
		selfIntroduction: 'Self Introduction',
		deliverables: 'Deliverables',
		hobbies: 'Hobbies',
		specialSkills: 'Special Skills',
		itSkills: 'IT Skills',
		otherSkills: 'Other Skills',
		advanced: 'Advanced',
		intermediate: 'Intermediate',
		beginner: 'Beginner',
		threeYearsOrMore: 'Three years or more',
		oneToOneAndHalfYears: 'One to one and a half years',
		lessThanThreeYears: 'Less than three years',
		basic: 'Basic',
		editProfile: 'Edit Profile',
		skillExists: 'Skill already exists at this level!',
		skillAdded: 'Skill added successfully!',
		skillDeleted: 'Skill deleted successfully!',
		selectSkill: 'Select Skill',
		skill: 'Skill',
		level: 'Level',
		languageName: 'Language Name',
		awardName: 'Award/Certificate Name',
		noLanguageSkillsEdit: 'No skills added yet',
		noSkillsEdit: 'No skills added yet. Use the form above to add skills.',
		noLanguageSkills: 'No skills available',
		noAwardsEdit: 'No awards added yet',
		noAwards: 'No awards available',
		levelInfo: 'Advanced: Over 3 years | Intermediate: Less than 3 years | Beginner: Less than 1 year',
		notEntered: 'Not entered',
		not_submitted: 'Not submitted',
		changed: 'Changed',
		none: 'None',
		// ChangedFieldsModal
		changed_elements_list: 'Changed Elements List',
		changes_count_message: 'Total %d change(s)',
		changes_count_short: '%d change(s)',
		no_changes: 'No changes',
		category_basic_info: 'Basic Info',
		category_skills_abilities: 'Skills & Abilities',
		category_interests: 'Hobbies & Interests',
		category_certifications: 'Qualifications',
		category_portfolio: 'Portfolio',
		category_qa: 'Q&A',
		category_other: 'Other',
		gallery: 'Gallery',
		hobbies_description: 'Hobbies Description',
		special_skills_description: 'Special Skills Description',
		showMore: 'Show more',
		showLess: 'Show less',
		// Deliverables translations
		addDeliverable: 'Add Deliverable',
		createDeliverable: 'Create Deliverable',
		editDeliverable: 'Edit Deliverable',
		noDeliverables: 'No deliverables yet',
		addFirstDeliverable: 'Add your first deliverable',
		deliverableCreated: 'Deliverable created successfully!',
		deliverableUpdated: 'Deliverable updated successfully!',
		deliverableDeleted: 'Deliverable deleted successfully!',
		titleRequired: 'Title is required',
		filesRequired: 'At least one image is required',
		selectImages: 'Select Images',
		addMoreImages: 'Add More Images',
		replaceImages: 'Replace Images',
		currentImages: 'Current Images',
		newImages: 'New Images',
		images: 'images',
		view: 'View',
		openLink: 'Open Link',
		openCodeLink: 'Open Code Link',
		allImages: 'All Images',
		confirmDelete: 'Are you sure you want to delete this deliverable?',
		levels: {
			上級: 'Advanced: more than 3 years',
			中級: 'Intermediate: less than 3 years',
			初級: 'Beginner: less than 1 year',
			JLPT: 'JLPT',
			JDU_JLPT: 'JDU Japanese Certification Test',
			IELTS: 'IELTS',
			日本語弁論大会学内: 'Japanese Speech Contest',
			ITコンテスト学内: 'IT contest',
		},
		approved: 'Approved',
		disapproved: 'Disapproved',
		resubmission_required: 'Resubmission Required',
		submitted: 'Submitted',
		draft: 'Draft',
		published: 'Public',
		private: 'Private',
		notifications: 'Notifications',
		mark_all_read: 'Mark All Read',
		mark_as_read: 'Mark as read',
		readed: 'Read',
		unread: 'Unread',
		read: 'Read',
		expand: 'Expand',
		collapse: 'Collapse',
		no_notifications: 'No notifications yet...',
		no_unread_notifications: 'No unread notifications yet...',
		no_read_notifications: 'No read notifications yet...',
		new: 'NEW',
		ok: 'OK',
		delete: 'Delete',
		loading: 'Loading...',
		warning: 'Warning',
		confirmApprove: 'Approve?',
		confirmSendBack: 'Send Back?',
		confirmApproveDesc: 'This action will send an "approved" notification to the student. Do you want to continue?',
		confirmSendBackDesc: 'This action will send a rejection notification with comments to the student. Do you want to continue?',
		staffFeedbackTitle: 'Staff feedback',
		staffFeedbackHint: 'Please update your profile based on this feedback and resubmit.',
		pastStaffComments: 'Past staff comments',
		staffComment: 'Comment',
		resubmissionComment: 'Return comment',
		approve: 'Approve',
		disapprove: 'Do not approve',
		sendBack: 'Send back',
		originPlaceholder: 'Uzbekistan',
		addressFuriganaPlaceholder: 'ウズベキスタン',
		studentNotApprovedByStaff: 'Student must be approved by staff',
		profileVisibilityEnabled: 'Profile visibility enabled successfully',
		profileHidden: 'Profile hidden successfully',
		errorSettingVisibility: 'Error setting visibility',
		navigationWarningTitle: 'Unsaved Changes',
		navigationWarningMessage: 'You have unsaved changes. Do you want to discard them and navigate away?',
		welcome: 'JDU Portfolio',
		forgotPassword: 'Forgot Password',
		resetPassword2: 'Enter your email address to receive password reset instructions',
		enterYourLogin: 'Enter your email',
		enterYourPassword: 'Enter your password',
		mailAddress: 'Email Address',
		send: 'Send Email',
		loginLabel: 'Login',
		profile_publish_request: 'Application for Publication',
		profile_publish_explanation: `We will release your profile on the JDU Portfolio Service upon your application for the purpose of facilitating your job search.
For the purpose of facilitating your job search, your profile will be published on the JDU Portfolio Service based on your application.

After your application is submitted, it will be sent to the JDU Japan Employment Department, where a JDU staff member will review the contents.
If you have any questions or concerns about the content of your profile, we will contact you by email.
If there are no questions or concerns, your application will be approved and your profile will be made public.

Please make sure that your profile does not fall under any of the prohibited activities listed below, and submit your request for publication by clicking the "Submit" button.`,
		prohibited_actions: 'Prohibited acts',
		prohibited_actions_content: `
		JDU students are prohibited from engaging in the following acts when using this service:
<ol>
<li>Providing false or inaccurate information</li>
<li>Defaming, slandering, threatening individuals or organizations, or engaging in acts likely to lead to such outcomes</li>
<li>Infringing or potentially infringing on the rights of others, including copyrights, trademarks, or other intellectual property rights. Example: Unauthorized use of photos, works, or characters of celebrities or others</li>
<li>Interfering with the smooth operation of this service, or damaging or potentially damaging the reputation or honor of the company or job seekers. Example: Direct contact or interviews with job seekers</li>
<li>Engaging in criminal acts, or acts that violate laws or public order and morals, or acts likely to lead to such outcomes</li>
<li>Being involved with anti-social forces, providing benefits to or using such forces, or maintaining inappropriate relationships with them, or engaging in acts likely to lead to such outcomes</li>
</ol>`,
		confirm_no_prohibited_actions: 'I have confirmed that the above-mentioned prohibited acts do not apply to me.',
		no_button: 'No',
		apply_button: 'Submit',
		// Missing keys for Top.jsx
		draftSubmittedSuccessfully: 'Draft submitted successfully!',
		noDraftToSubmit: 'No draft to submit',
		errorSubmittingDraft: 'Error submitting draft',
		noDataFound: 'No data found',
		start_checking: 'Start checking',
		addHobby: 'Add Hobby',
		addSpecialSkill: 'Add Special Skill',
		addDescription: 'Add Description',
		addTag: 'Add Tag',
		hobbiesDetailDescription: 'Detailed description of hobbies',
		specialSkillsDetailDescription: 'Detailed description of special skills',
		hobbiesTagPlaceholder: 'Please enter hobby tags',
		specialSkillsTagPlaceholder: 'Please enter special skill tags',
		hobbiesDescriptionPlaceholder: 'Please describe your hobbies in detail',
		specialSkillsDescriptionPlaceholder: 'Please describe your special skills in detail',
		hobbiesTags: 'Hobby Tags',
		specialSkillsTags: 'Special Skill Tags',
		// Missing keys for ChekProfile.jsx
		statusUpdatedSuccessfully: 'Status updated successfully',
		errorUpdatingStatus: 'Error updating status',
		checking: 'Checking',
		not_approved_yet: 'Not approved yet',
		approvalStatus: 'Approval Status',
		approval_status_unconfirmed: 'Unconfirmed',
		approval_status_in_review: 'In review',
		approval_status_returned: 'Returned',
		approval_status_approved: 'Approved',
		visible_status: 'Visibility Status',
		visible: 'Visible',
		invisible: 'Invisible',
		// Missing keys for Setting.jsx
		personal_info: 'Personal Information',
		// Missing keys for general usage
		search_placeholder: 'Search by name, furigana, or ID...',
		search_filters: 'Search filters',
		no_matches_found: 'No matches found',
		start_typing: 'Start typing to see suggestions',
		// Missing keys for hardcoded text
		entrance: 'Entrance',
		graduation: 'Graduation',
		credits_unit_short: 'Credits',
		qualifications: 'Qualifications',
		student_grades: 'Student Grades',
		specialized_knowledge: 'Specialized Knowledge',
		personality: 'Personality',
		work_experience: 'Work Experience',
		career_goals: 'Career Goals',
		comments: 'Comments',
		company_introduction_video: 'Introduction Video',
		pleaseAnswerRequired: 'Please answer all required questions',
		student_label: 'Student',
		age_label: 'Age',
		age_unit: ' years',
		application_date: 'Application Date',
		application_count: 'Application Count',
		confirmation_status: 'Confirmation Status',
		unconfirmed: 'Unconfirmed',
		confirmed: 'Confirmed',
		company_video_url_placeholder: 'e.g., https://youtu.be/rSRpRd1E45w?si=3r7PqVgWt67ZA2i5',
		save_button: 'Save',
		no_video_set: 'No video has been set',
		company_introduction_video_description: 'You can view the company introduction video provided by the employer',
		new_target_audience_add: 'Add new target audience',
		required_skills_placeholder: 'e.g., Practical experience with HTML/CSS/JavaScript (3+ years)',
		preferred_skills_placeholder: 'e.g., Practical experience with TypeScript',
		// CompanyProfile page translations
		business_content_placeholder: 'Enter business content',
		recruitment_requirements: 'Recruitment Requirements',
		target_audience: 'Target',
		target_person: 'Target Person',
		target_audience_placeholder: 'Enter target audience',
		additional_info: 'Additional Information',
		work_location_placeholder: 'Enter work location',
		work_hours: 'Work Hours',
		work_hours_placeholder: 'Enter work hours',
		salary_placeholder: 'Enter salary',
		benefits_placeholder: 'Enter benefits',
		allowances_placeholder: 'Enter other allowances (welfare)',
		selection_process_placeholder: 'Enter selection process',
		established: 'Established',
		established_placeholder: 'Enter established date',
		employee_count: 'Employee Count',
		employee_count_placeholder: 'Enter employee count',
		location_placeholder: 'Enter location',
		company_description_placeholder: 'Enter company description',
		saving: 'Saving...',
		// Language settings translations
		unsaved_changes_title: 'Unsaved Changes',
		language_change_unsaved_message: 'You have unsaved changes. Would you like to save them before changing the language?',
		save_and_continue: 'Save and Continue',
		discard_changes: 'Discard Changes',
		display_language: 'Display Language',
		language_settings: 'Language Settings',
		language_change_notice: 'This will change the application display language',
		// CreateSkill page translations
		it_skills_manager: 'Skills Manager',
		search_skills: 'Search skills...',
		add_new_skill: 'Add New Skill',
		skills: 'Skills',
		no_skills_found: 'No skills found',
		no_skills_available: 'No skills available',
		try_adjusting_search: 'Try adjusting your search terms',
		start_adding_skill: 'Start by adding your first IT skill',
		edit_skill: 'Edit Skill',
		skill_name: 'Skill Name',
		choose_color: 'Choose Color',
		color_code: 'Color Code',
		update: 'Update',
		create: 'Create',
		confirm_delete: 'Confirm Delete',
		delete_skill_confirmation: 'Are you sure you want to delete this skill? This action cannot be undone.',
		failed_fetch_skills: 'Failed to fetch skills',
		skill_name_required: 'Skill name is required',
		skill_updated_success: 'Skill updated successfully',
		skill_created_success: 'Skill created successfully',
		failed_save_skill: 'Failed to save skill',
		skill_deleted_success: 'Skill deleted successfully',
		failed_delete_skill: 'Failed to delete skill',
		// Recruiter file translations
		company_documents: 'Documents',
		upload_info: 'You can upload up to 3 files (PDF, Word, Excel, PowerPoint). Maximum total size: 20MB',
		select_files: 'Select Files',
		selected_files: 'Selected files',
		uploading: 'Uploading...',
		upload: 'Upload',
		max_files_info: 'You have reached the maximum number of files (3). Please delete existing files to upload new ones.',
		max_files_reached: 'Maximum files reached (3 files)',
		can_upload_only: 'Can upload only',
		more_files: 'more file(s)',
		invalid_file_type: 'Invalid file type',
		insufficient_space: 'Insufficient space. Available',
		available: 'Available',
		space_used: 'Space used',
		file_too_large: 'File too large',
		total_size_exceeded: 'Total size exceeded 20MB',
		files_uploaded_successfully: 'Files uploaded successfully',
		upload_failed: 'Upload failed',
		no_files_uploaded: 'No files uploaded yet',
		no_intro_pages_set: 'No intro pages have been set',
		download_failed: 'Download failed',
		delete_file: 'Delete File',
		delete_file_confirm: 'Are you sure you want to delete this file?',
		deleting: 'Deleting...',
		file_deleted: 'File deleted successfully',
		delete_failed: 'Delete failed',
		addNews: 'Add News',
		noNewsFound: 'No news found',
		noNewsAvailable: 'No news available',
		tryAdjusting: 'Try adjusting your search terms',
		noImageAvailable: 'No Image Available',
		confirmDeleteNews: 'Are you sure you want to delete this news?',
		hashtagPlaceholder: 'e.g., technology, news, update',
		updateNews: 'Update',
		filter: 'Filter',
		clear: 'Clear',
		apply: 'Confirm',
		noSkills: 'No skills available.',
		filter_all: 'All',
		filter_required: 'Required',
		filter_optional: 'Optional',
		sort_required_first: 'Required first',
		add_required: 'Add Required',
		add_optional: 'Add Optional',
		// News page UI/UX additions
		readMore: 'Read more',
		close: 'Close',
		retry: 'Retry',
		source: 'Source',
		sortBy: 'Sort by',
		newest: 'Newest',
		oldest: 'Oldest',
		searchNewsPlaceholder: 'Search news... ',
		loadMore: 'Load more',
		newsDetails: 'News details',
		confirmMarkAllRead: 'Mark all news as read?',
		allNewsMarkedRead: 'All news marked as read.',
		login_failed: 'Login failed. Please check your email and password.',
		no_data_found: 'No data found.',
		next: 'Next',
		expectedGraduationYearMonth: 'Expected Graduation Month',
		number_of_applications: 'Number of Applications',
		changed_elements: 'Changed Elements',
		address_furigana: 'Address (Furigana)',
		education: 'Education',
		add_arubaito: 'Add Part-time Job',
		arubaito: 'Part-time Job',
		no_arubaito: 'No part-time jobs added yet.',
		licenses: 'Licenses & Certifications',
		no_licenses: 'No licenses or certifications added yet.',
		additional_info: 'Additional Information',
		additional_address: 'Additional Address',
		additional_address_furigana: 'Additional Address (Furigana)',
		additional_email: 'Additional Email',
		additional_phone: 'Additional Phone',
		additional_index: 'Additional Index',
		isMarried: 'Marital Status',
		married: 'Married',
		single: 'Single',
		postal_code: 'Postal Code',
		// Profile empty states & forms (Education, Work Experience, etc.)
		no_education: 'No education records yet.',
		no_work_experience: "You don't have any work experience yet.",
		edit_education: 'Edit Education',
		add_education: 'Add Education',
		edit_work_experience: 'Edit Work Experience',
		add_work_experience: 'Add Work Experience',
		present: 'Present',
		company: 'Company',
		role_position: 'Role/Position',
		description_label: 'Description',
		from_yyyy_mm: 'From (YYYY-MM)',
		to_yyyy_mm: 'To (YYYY-MM)',
		work_experience_from_placeholder: '2023-01',
		work_experience_to_placeholder: '2024-01 or leave empty',
		institution: 'Institution',
		status: 'Status',
		year: 'Year',
		month: 'Month',
		edit_arubaito: 'Edit Part-time Work',
		add_arubaito_form: 'Add Part-time Work',
		edit_license: 'Edit License/Certificate',
		add_license_form: 'Add License/Certificate',
		certificate_name: 'Certificate Name',
		loading: 'Loading...',
		institution_placeholder: 'e.g. Tokyo University',
		status_placeholder: 'e.g. Graduated, Enrolled',
		year_placeholder: '2023',
		month_placeholder: '03 (optional)',
		certificate_placeholder: 'e.g. TOEIC 800, 運転免許証',
		company_placeholder: 'e.g. コンビニエンスストア',
		role_placeholder: 'e.g. レジ担当',
		period_placeholder: 'e.g. 2023-01 ~ 2024-01',
		period: 'Period',
		download_cv: 'Download CV',
		error_loading_data: 'Error loading data',
		error_fetching_draft: 'Error fetching draft data',
		unable_to_determine_student_id: 'Unable to determine student ID',
		loading_student_profile: 'Loading student profile...',
		loading_questions: 'Loading questions...',
		changes_cancelled: 'Changes cancelled',
		item_deleted_success: 'Item deleted successfully!',
		error_deleting_item: 'Error deleting item. Please try again.',
		error_label: 'Error',
		no_valid_student_id: 'No valid student ID found',
		// Company profile & shared
		invalid_video_urls_removed: 'Some invalid video URLs were removed.',
		max_three_videos: 'You can add up to 3 videos.',
		upload_failed_file_invalid: 'File upload failed. Please check file size and format.',
		required: 'Required',
		add_role: 'Add Role',
		url_placeholder: 'https://example.com',
		url_placeholder_page: 'https://example.com/page',
		credit_details_disabled: 'Credit details page is currently disabled',
		credit_details_load_error: 'Credit details could not be loaded',
		credit_details_sync_success: 'Credit details synced successfully',
		credit_details_sync_failed: 'Failed to sync credit details',
		file_size_max_5mb: 'File size too large. Maximum size is 5MB.',
		invalid_file_type_images_only: 'Invalid file type. Please select image files only.',
		video_thumbnail: 'Video thumbnail',
		company_intro_video: 'Company Introduction Video',
		video_thumbnails: 'Video thumbnails',
		close: 'Close',
		close_filter_modal: 'Close filter modal',
		more: 'More',
		delete_aria: 'Delete',
		download_aria: 'Download',
		enrollment: 'Enrollment',
		graduation: 'Graduation',
		jdu: 'JDU',
		// Mail Service
		ms_title: 'Mail Service',
		ms_periodic_email: 'Periodic Email',
		ms_inactive_student_email: 'Inactive Student Email',
		ms_periodic_email_settings: 'Periodic Email Settings',
		ms_send_period: 'Send Period',
		ms_search_period: 'Search Period',
		ms_email_subject: 'Email Subject',
		ms_email_body: 'Email Body',
		ms_plain_text_hint: 'Write in plain text. Line breaks are preserved.',
		ms_inactive_description: 'Search for students who were returned for resubmission and have not resubmitted within the selected week interval. Each interval is exclusive (e.g. 1 week = 7–13 days ago only).',
		ms_inactive_period_title: 'Returned Students (Interval-based)',
		ms_1_week: '1 week',
		ms_2_weeks: '2 weeks',
		ms_3_weeks: '3 weeks',
		ms_4_weeks: '4 weeks',
		ms_1_month: '1 month',
		ms_3_months: '3 months',
		ms_6_months: '6 months',
		ms_9_months: '9 months',
		ms_1_year: '1 year',
		ms_1_year_plus: '1 year & more',
		ms_6_months_plus: '6 months & more',
		ms_save: 'Save',
		ms_search: 'Search',
		ms_reset: 'Reset',
		ms_send_email: 'Send Email',
		ms_students_found: 'Students found',
		ms_no_students_found: 'No students found for this period',
		ms_student_id: 'Student ID',
		ms_student_name: 'Name',
		ms_email: 'Email',
		ms_last_updated: 'Last Updated',
		ms_last_updated_by: 'Last updated by',
		ms_last_activity: 'Last Activity',
		ms_never_active_title: 'Never Active Students',
		ms_never_active_description: 'Find students who have never submitted a single draft since registration.',
		ms_registered_at: 'Registered',
		ms_never_active_students: 'Never active students',
		ms_never_active_hint: '(have never submitted a single draft since registration)',
		ms_setting_updated: 'Setting updated successfully',
		ms_error_loading: 'Error loading settings',
		ms_error_updating: 'Error updating setting',
		ms_error_searching: 'Error searching students',
		ms_error_sending: 'Error sending emails',
		ms_fill_all_fields: 'Please fill in all fields',
		ms_emails_sent: 'Emails sent',
		ms_send_result: 'Send result',
		ms_successful: 'successful',
		ms_failed: 'failed',
		ms_year: 'year',
		ms_month: 'month',
		ms_week: 'week',
		ms_day: 'day',
		ms_days: 'days',
		ms_schedule_day_of_month: 'Send Day (of month)',
		ms_schedule_hour: 'Send Hour',
		ms_schedule_day_of_month_hint: 'Day of the month to send (1–28)',
		ms_schedule_hour_hint: 'Hour to send (0–23)',
		// Sidebar & profile header
		maintenance: 'Maintenance',
		expected_graduation_month: 'Expected Graduation',
		enrolled_partner_university: 'Partner University',
		not_set: 'Not set',
		getLink: 'Get Link',
		generating: 'Generating...',
		regenerateLink: 'New Link',
		linkExpiresIn: 'Expires in',
		link_expired: 'Link expired',
		link_expiry_notice: 'Link validity',
		link_generated_success: 'Link generated successfully!',
		link_generation_failed: 'Failed to generate link',
		link_copied: 'Link copied!',
		linkCopied: 'Link copied!',
		profile_not_public: 'Make your profile public first',
		deactivate_link: 'Deactivate link',
		deactivating: 'Deactivating...',
		link_deactivated_success: 'Link deactivated',
		link_deactivate_failed: 'Failed to deactivate link',
		confirm_to_download_cv: 'I have read it',
		resumeDownloadWarning: `⚠️ Please read carefully before downloading your CV!
Dear Students,
This CV is automatically generated by the system based on the information in your website portfolio. Therefore, the content may not be 100% accurate. After downloading, please be sure to compare it with the "見本 履歴書" (Sample CV) and manually check the content yourself!
❗️ Common mistakes to watch out for:
1) About "職務経歴書" (Work History):
In the first table section, only include projects you participated in through the university's "Cowork" system. Because the data is generated automatically, it might be mixed with other information. Please ensure that "Cowork" projects are strictly separated from outside experiences (part-time jobs, freelance work, other companies' projects, etc.) and that each is written in its correct section.

2) Addresses – "現住所" and "連絡先":
"現住所" (Current Address): Write the address where you currently live.
"連絡先" (Contact Address): Write the address where your guardians (parents, etc.) live.
If both addresses are the same, simply write "同上" (Same as above) in the bottom section (Contact Address). Please check that this is entered correctly.

3) "自己PR" (Self PR):
You must not leave this section as the auto-generated template. You need to rewrite it yourself every time, adapting it to the company or situation you are applying for. Please check that it has been properly customized in your own words.
💡 If you have any questions or problems editing your CV, please do not hesitate to ask your senpais or university staff for help.`,
		companies: 'Companies',
		add_recruiter: 'Add Recruiter',
		choose_existing_company: 'Choose Existing Company',
		create_new_company: 'Create New Company',
		confirm_delete_recruiter: 'Are you sure you want to delete this recruiter?',
		not_assign_company: 'This recruiter is not assigned to any company.',
		companies: 'Companies',
		new_company: 'New Company',
		company_name: 'Company Name',
		company_name_placeholder: 'Search by company name, address, or recruiter name...',
		is_partner_true: 'Partner',
		is_partner_false: 'Non-Partner',
		company_address: 'Address',
		recruiters: 'Recruiters',
		no_companies_found: '企業が見つかりません',
		is_partner: 'Partner',
		partner_chip: 'Partner',
		set_parent_recruiter: 'Assigned as parent recruiter',
		removed_parent_recruiter: 'Removed parent recruiter',
		assign_recruiter: 'Assign Recruiter',
		create_company: 'Create Company',
		company_identity: 'Company Identity',
		no_recruiters_found: 'No recruiters found',
		search_recruiters: 'Search by recruiter name, email, or company name...',
		select_company: 'Select Company',
		company_not_assigned_title: 'No Company Assigned',
		company_not_assigned_desc: 'You have not been assigned to any company yet. Please contact the administrator.',
		errorFetchingData: 'Failed to load data. Please try again.',
		assigned_recruiter_to_company: 'Recruiter assigned to company successfully',
		unassigned_recruiter_from_company: 'Recruiter unassigned from company successfully',
		updated_company_identity: 'Company identity updated successfully',
		updated_company_profile: 'Company profile updated successfully',
		details: 'Details',
		edit_recruiter: 'Edit Recruiter',
		company_change_hint: 'Company can only be changed from the company page',
	},
	ja: {
		age: '歳',
		tagline: '一行PR',
		company_information: '会社情報',
		company_name: '会社名',
		company_Address: '所在地',
		company_website: 'ホームページ',
		company_description: '企業概要',
		company_overview: '企業概要',
		business_overview: '事業内容',
		employee_Count: '従業員数',
		established_Date: '設立',
		company_capital: '資本金',
		company_revenue: '売上高',
		company_representative: '代表者',
		job_details: '募集概要',
		job_title: '募集職種',
		job_description: '仕事内容',
		japanese_level: '応募要件：日本語レベル',
		application_requirements_other: '応募要件：その他',
		required_skills: '必須：経験やスキル',
		welcome_skills: 'ようこそ：経験やスキル',
		recommended_skills: '推奨：経験やスキル',
		recommended_licenses: '推奨：資格や免許',
		recommended_other: '推奨：その他',
		number_of_openings: '募集人数',
		employment_type: '雇用形態',
		probation_period: '試用期間',
		employment_period: '雇用期間',
		work_location: '勤務地',

		compensation_benefits: '給与・待遇・福利厚生',
		salary: '給与',
		salary_increase: '昇給',
		bonus: '賞与',
		allowances: 'その他手当\n福利厚生',
		holidays_vacation: '休日・休暇',
		benefits: '社会保険',

		other: 'その他',
		other_notes: 'その他',

		selection_info: '選考情報',
		selection_process: '採用までの流れ',
		target_audience: '対象',
		interview_method: '面接対応',
		// 紹介ページセクション
		intro_page: '紹介ページ',
		intro_page_thumbnail: '紹介ページ',
		intro_page_links: '紹介ページのリンク',
		newIntroLinkTitle: 'リンクタイトル',
		newIntroLinkUrl: 'リンクURL',
		intro_link_title: 'リンクタイトル',
		max_four_links: '最大4件まで追加できます',
		// 人事オプション
		retirement_benefit: '退職金',
		telework_availability: 'テレワークの有無',
		housing_availability: '寮、社宅等の有無',
		relocation_support: '航空券代・赴任費用の負担',
		airport_pickup: '来日時の送迎',
		rank: '位',
		title: 'タイトル',
		description: 'デスクリプション',
		sourseLink: '出典リンク',
		hashtag: 'ハッシュタグ',
		CreateNews: 'ニュース作成',
		EditNews: 'ニュース変更',
		visibleToRecruiter: 'リクルーターに表示',
		newsVisibility: 'ニュース表示設定',
		universityNews: '大学ニュース',
		universityNewsDescription: '全員に表示',
		recruiterNews: 'リクルーターニュース',
		recruiterNewsDescription: 'リクルーター以外に表示',
		imageRequired: '画像は必須です',
		descriptionRequired: '説明は必須です',
		fillRequiredFields: '必須項目（タイトル、説明、画像）を入力してください',
		cencel: 'キャンセル',
		searchByHashtag: 'ハッシュタグ、会社名、ニュースのタイトルで検索します...',
		checkOut: 'ニュース一覧',
		newsHighlights: 'ニュース一覧',
		news: 'ニュース',
		Qualifications: '資格・表彰暦等',
		studetentSkillManagment: '学生スキル管理',
		origin: '出身地',
		major: '専攻',
		jobType: '希望職種',
		qualification: '資格',
		japaneseSpeechContest: '日本語弁論大会学内',
		itContest: 'ITコンテスト学内',
		languageSkills: '言語スキル',
		japanese: '日本語',
		confirmationStatus: '確認状況',
		published: '公開中',
		private: '非公開',
		// Visibility filter options
		visible: '公開',
		invisible: '非公開',
		// Approval status filter options
		submitted: '未確認',
		checking: '確認中',
		resubmission_required: '要修正',
		disapproved: '差し戻し',
		approved: '確認済',
		pleaseAnswerRequired: '必須の質問に回答してください',
		// News page UI/UX additions
		readMore: '続きを読む',
		close: '閉じる',
		retry: '再試行',
		source: '出典',
		sortBy: '並び替え',
		newest: '新しい順',
		oldest: '古い順',
		searchNewsPlaceholder: 'ニュースを検索...',
		loadMore: 'もっと見る',
		newsDetails: 'ニュース詳細',
		confirmMarkAllRead: 'すべてのニュースを既読にしますか？',
		allNewsMarkedRead: 'すべてのニュースを既読にしました。',
		start_checking: '確認開始',
		start_check_drafting: '確認開始',
		setToChecking: 'ステータスが確認中に更新されました',
		studentCredits: '学生単位数',
		general: '一般',
		home: 'ホーム',
		profile: 'プロフィール',
		student_search: '学生検索',
		student_check: 'プロフィール確認',
		staff: '職員',
		recruiter: 'リクルーター',
		bookmarked: '気になる',
		settings: '設定',
		help: 'ヘルプ',
		student_qa: '学生質問管理',
		logout: 'ログアウト',
		Are_you_sure: 'ログアウトしてもよろしいですか？',
		yesModal: 'はい',
		noModal: 'いいえ',
		jdu_portfolio: 'JDU ポートフォリオ',
		japan: '日本',
		uzbekistan: 'ウズベキスタン',
		edit: '編集',
		saveDraft: '新規保存',
		updateDraft: '上書き保存',
		submitAgree: '承認依頼・同意',
		next_button: '学生検索へ ➜',
		large_class_photo_alt: '大きなクラスの写真',
		group_photo_alt: 'グループの写真',
		changes_saved: '変更が正常に保存されました！',
		grade: '学年',
		grade1: '1年生',
		grade2: '2年生',
		grade3: '3年生',
		grade4: '4年生',
		programming_languages: 'プログラミング言語',
		any: 'いずれか',
		all: 'すべて',
		search_items: '検索...',
		select_all_filtered: 'すべて選択（絞り込み）',
		clear_filtered: 'クリア（絞り込み）',
		selected: '選択済み',
		select_all: 'すべて選択',
		clear_all: 'すべて解除',
		jlpt: '日本語能力試験',
		ielts: 'IELTS',
		jdu_certification: 'JDU日本語認定試験',
		credits: '単位数',
		credits_unit: '単位内',
		partner_university: '提携大学',
		special_skills: '特技 (有無)',
		yes: '有り',
		no: '無し',
		tokyo_communication_university: '東京通信大学',
		kyoto_tachibana_university: '京都橘大学',
		sanno_university: '産業能率大学',
		sanno_junior_college: '自由が丘産能短期大学',
		niigata_sangyo_university: '新潟産業大学',
		otemae_university: '大手前大学',
		okayama_university_of_science: '岡山理科大学',
		unaffiliated: '未所属',
		student: '学生',
		rows_per_page: '表示件数:',
		search: '検索',
		reset: '戻る',
		name_search: '氏名の検索',
		department: '部署',
		position: '役職',
		phone_number: '電話番号',
		name: '名前',
		sync: '同期',
		user_avatar: 'ユーザーアバター',
		company_logo: '会社ロゴ',
		upload_picture: '画像をアップロード',
		user: 'ユーザー',
		first_name: '名',
		first_name_furigana: '名 (フリガナ)',
		last_name: '姓',
		last_name_furigana: '姓 (フリガナ)',
		furigana_help: 'ひらがな・カタカナで読み方を入力してください',
		phone: '電話番号',
		email: 'メール',
		submit_date: '申請日',
		submit_count: '申請回数',
		check_status: '確認状況',
		visible_status: '公開状況',
		action: 'アクション',
		change_password: 'パスワードの変更',
		current_password: 'パスワード',
		new_password: '新しいパスワード',
		confirm_password: 'パスワードを認証する',
		toggle_password_visibility: 'パスワードの表示切り替え',
		contact_info: 'コンタクト情報',
		contact_email: 'メール',
		contact_phone: '電話番号',
		working_hours: '労働時間',
		location: '位置',
		personal_info: '個人情報',
		user_fetch_error: 'ユーザーデータの取得に失敗しました。',
		profile_update_success: 'プロフィールが更新されました。',
		profile_update_failed_retry: 'プロフィールの更新に失敗しました。再試行してください。',
		unknown_role_error: '不明な役割が検出されました。',
		password_mismatch: 'パスワードが一致しません。',
		current_password_required: '現在のパスワードを入力してください。',
		unknown_role: '不明な役割です。',
		fetch_user_failed: 'ユーザーデータの取得に失敗しました。',
		profile_updated: 'プロフィールが更新されました。',
		profile_update_failed: 'プロフィールの更新に失敗しました。再試行してください。',
		sync_successful: '同期に成功しました。',
		notFoundTitle: '404 - ページが見つかりません',
		notFoundMessage: 'お探しのページは見つかりませんでした。',
		backToHome: 'ホームへ戻る',
		maintenanceTitle: 'メンテナンス中',
		checkingMaintenance: 'メンテナンス状況を確認中...',
		checkingStatus: '自動的に再チェック中...',
		// Maintenance page translations
		maintenance_announcement: 'メンテナンスのお知らせ',
		announcement_settings: 'お知らせ設定',
		message: 'メッセージ',
		enter_announcement_message: 'お知らせメッセージを入力してください...',
		visibility_settings: '表示設定',
		make_public: 'ログインページで全ユーザーに表示する',
		announcement_visible_to_all_users: 'このお知らせは現在、ログインページで全ユーザーに表示されています。',
		announcement_hidden_from_users: 'このお知らせは現在、ユーザーに非表示になっています。',
		created_at: '作成日時',
		last_updated: '最終更新日時',
		message_required: 'メッセージは必須です',
		announcement_updated: 'お知らせが正常に更新されました',
		error_updating_announcement: 'お知らせの更新中にエラーが発生しました',
		please_update_announcement_first: 'まずお知らせを更新してください',
		announcement_made_public: 'お知らせは現在、全ユーザーに表示されています',
		announcement_made_private: 'お知らせは現在、ユーザーに非表示になっています',
		error_toggling_status: 'ステータスの切り替え中にエラーが発生しました',
		unauthorizedTitle: '403 - アクセスが拒否されました',
		unauthorizedMessage: 'このページにアクセスする権限がありません。',
		unauthorizedButton: 'ログインページへ戻る',
		back: '戻る',
		add: '追加',
		edit_profile: 'プロフィールを編集',
		liveProfile: '公開版',
		draftProfile: '編集版',
		qa_edit: 'QAを編集',
		faq_heading: 'よくある質問',
		faq_no_answer: '回答なし',
		q_edit: '質問を編集',
		age: '年齢',
		student_id: '学籍番号',
		top: 'トップ',
		qa: 'Q&A',
		stats: '単位数とスキル',
		errorFetchingStudent: '学生データの取得中にエラーが発生しました。',
		changesSavedSuccessfully: '変更が正常に保存されました！',
		errorSavingChanges: '変更の保存中にエラーが発生しました。',
		errorUploadingFile: 'ファイルのアップロード中にエラーが発生しました。',
		savingChanges: '変更を保存中...',
		changesSaved: '変更が保存されました',
		dataRecovered: 'データが正常に復元されました',
		recoverUnsavedChanges: '未保存の変更を復元しますか？',
		unsavedChangesFound: '前回の編集セッションから未保存の変更が見つかりました。復元しますか？',
		lastModified: '最終更新',
		discard: '破棄',
		restore: '復元',
		unsavedChangesTitle: '未保存の変更',
		unsavedChangesMessage: '未保存の変更があります。本当に破棄しますか？',
		continueEditing: '編集を続ける',
		discardChanges: '変更を破棄',
		unsavedChangesLanguageTitle: '言語を切り替える前に変更を保存しますか？',
		unsavedChangesLanguageMessage: '未保存の変更があります。言語を変更する前に保存しますか？',
		discardAndSwitch: '破棄して切り替え',
		saveAndSwitch: '保存して切り替え',
		dataRestoredAfterLanguageSwitch: '言語切り替え後、データが復元されました',
		unsavedChangesNavigationTitle: 'ページを離れる前に変更を保存しますか？',
		unsavedChangesNavigationMessage: '未保存の変更があります。このページを離れる前に保存しますか？',
		discardAndLeave: '破棄して離れる',
		saveAndLeave: '保存して離れる',
		fileUploadSuccess: 'ファイルのアップロードが完了しました。',
		selfIntroduction: '自己紹介',
		deliverables: '成果物',
		hobbies: '趣味',
		specialSkills: '特技',
		itSkills: 'ITスキル',
		otherSkills: 'その他の資格・免許・表彰暦等',
		// Deliverables translations
		addDeliverable: '成果物を追加',
		createDeliverable: '成果物を作成',
		editDeliverable: '成果物を編集',
		noDeliverables: 'まだ成果物がありません',
		addFirstDeliverable: '最初の成果物を追加',
		deliverableCreated: '成果物が正常に作成されました！',
		deliverableUpdated: '成果物が正常に更新されました！',
		deliverableDeleted: '成果物が正常に削除されました！',
		titleRequired: 'タイトルは必須です',
		filesRequired: '少なくとも1つの画像が必要です',
		selectImages: '画像を選択',
		addMoreImages: '画像を追加',
		replaceImages: '画像を置き換え',
		currentImages: '現在の画像',
		newImages: '新しい画像',
		images: '画像',
		view: '表示',
		openLink: 'リンクを開く',
		openCodeLink: 'コードリンクを開く',
		allImages: 'すべての画像',
		confirmDelete: 'この成果物を削除してもよろしいですか？',
		上級: '上級',
		中級: '中級',
		初級: '初級',
		threeYearsOrMore: '3年間以上',
		oneToOneAndHalfYears: '1年間〜1年間半',
		lessThanThreeYears: '3年未満',
		basic: '基礎',
		loading: '読み込み中...',
		save: '保存',
		cancel: 'キャンセル',
		editProfile: 'プロフィールを編集',
		skillExists: 'このレベルにはスキルが既に存在します！',
		skillAdded: 'スキルが正常に追加されました！',
		skillDeleted: 'スキルが削除されました！',
		selectSkill: 'スキルを選択',
		skill: 'スキル',
		level: 'レベル',
		languageName: '言語名',
		awardName: '賞・資格名',
		noLanguageSkillsEdit: 'スキルがまだ追加されていません',
		noSkillsEdit: 'スキルがまだ追加されていません。上のフォームから追加してください。',
		noLanguageSkills: 'スキルなし',
		noAwardsEdit: '賞・資格がまだ追加されていません',
		noAwards: '賞・資格なし',
		levelInfo: '上級：3年以上　　　　中級：3年未満　　　　初級：1年未満',
		notEntered: '未入力',
		not_submitted: '未提出',
		changed: '変更あり',
		none: 'なし',
		// ChangedFieldsModal
		changed_elements_list: '変更項目一覧',
		changes_count_message: '合計 %d 件の変更があります',
		changes_count_short: '%d 件の変更',
		no_changes: '変更なし',
		category_basic_info: '基本情報',
		category_skills_abilities: 'スキル・能力',
		category_interests: '趣味・興味',
		category_certifications: '資格・認定',
		category_portfolio: 'ポートフォリオ',
		category_qa: 'Q&A',
		category_other: 'その他',
		gallery: 'ギャラリー',
		hobbies_description: '趣味説明',
		special_skills_description: '特技説明',
		showMore: 'もっと見る',
		showLess: '閉じる',
		levels: {
			上級: '上級: 3年以上',
			中級: '中級: 3年未満',
			初級: '初級: 1年未満',
			JLPT: 'JLPT',
			JDU_JLPT: 'JDU日本語認定試験',
			IELTS: 'IELTS',
			日本語弁論大会学内: '日本語弁論大会学内',
			ITコンテスト学内: 'ITコンテスト学内',
		},
		not_approved_yet: '未承認',
		approvalStatus: '承認状況',
		approval_status_unconfirmed: '未確認',
		approval_status_in_review: '確認中',
		approval_status_returned: '差し戻し',
		approval_status_approved: '承認済',
		approved_draft: '承認済',
		disapproved_draft: '差し戻し',
		resubmission_required_draft: '修正して再申請してください',
		submitted_draft: '申請済',
		draft: '未申請',
		notifications: '通知',
		mark_all_read: 'すべて既読にする',
		mark_as_read: '既読にする',
		readed: '既読',
		unread: '未読',
		read: '既読',
		expand: '展開',
		collapse: '折りたたむ',
		no_notifications: 'まだ通知がありません...',
		no_unread_notifications: 'まだ未読の通知がありません...',
		no_read_notifications: 'まだ既読の通知がありません...',
		new: '新着',
		ok: 'OK',
		delete: '削除',
		warning: '警告',
		confirmApprove: '承認しますか？',
		confirmSendBack: '差し戻しますか？',
		confirmApproveDesc: 'この操作は学生に「承認済」通知を送ります。続行しますか？',
		confirmSendBackDesc: 'この操作は学生に差し戻し通知をコメント付きで送ります。続行しますか？',
		staffFeedbackTitle: 'スタッフからのフィードバック',
		staffFeedbackHint: 'このフィードバックをもとにプロフィールを修正し、再度提出してください。',
		pastStaffComments: '過去のスタッフコメント',
		staffComment: 'コメント',
		resubmissionComment: '差し戻しコメント',
		approve: '承認する',
		disapprove: '承認しない',
		sendBack: '差し戻し',
		originPlaceholder: 'Uzbekistan',
		addressFuriganaPlaceholder: 'ウズベキスタン',
		studentNotApprovedByStaff: '学生はスタッフによる承認が必要です',
		profileVisibilityEnabled: 'プロフィールの公開設定が正常に有効化されました',
		profileHidden: 'プロフィールが正常に非公開になりました',
		errorSettingVisibility: '公開設定エラー',
		navigationWarningTitle: '未保存の変更',
		navigationWarningMessage: '保存されていない変更があります。変更を破棄して移動しますか？',
		welcome: 'JDU Portfolio',
		forgotPassword: 'パスワードをお忘れですか',
		resetPassword2: 'パスワードリセット手順を受け取るメールアドレスを入力してください',
		enterYourLogin: 'メールアドレスを入力',
		enterYourPassword: 'パスワードを入力',
		mailAddress: 'メールアドレス',
		send: '送信',
		loginLabel: 'ログイン',
		profile_publish_request: '公開申請',
		profile_publish_explanation: `就職活動を円滑にする目的のために、あなたのプロフィールを申請に基づき、JDUポートフォリオサービス上に公開します。

公開申請後はJDU日本就業部に送信され、職員による内容確認が行われます。
記載内容について不明点や質問がある場合は、メールにてご連絡します。
記載内容について不明点や質問がない場合は、申請内容が承認され、あなたのプロフィールが公開されます。

あなたのプロフィールの内容等が以下の禁止行為のに該当しないこと確認し、申請ボタンから公開申請をしてください。`,
		prohibited_actions: '禁止行為',
		prohibited_actions_content: `JDUの学生は、本サービスの利用に際し、次の各号の行為を行ってはならないものとします。
<ol>
<li>虚偽または不正確な情報を提供する行為</li>
<li>個人や団体を誹謗、中傷、脅迫、またはそのおそれのある行為</li>
<li>著作権、商標権、その他の知的財産権を含む他人の権利を侵害し、またはそのおそれのある行為 例：芸能人や他人の写真、作品、キャラクターなどを無断で利用する行為</li>
<li>本サービスの円滑な運営を妨げる行為、または当社および求人者の信用または名誉を毀損する行為、もしくはそれらのおそれのある行為 例：求人者への直接連絡、面談等の行為</li>
<li>犯罪行為、または法令に反する行為等公序良俗に反する行為、もしくはそれらのおそれのある行為</li>
<li>反社会的勢力等に関連する組織に属する行為、反社会的勢力に利益を与え、または利用する等不適切な関係を持つ行為、もしくはそれらのおそれのある行為</li>
</ol>`,
		confirm_no_prohibited_actions: '上記の禁止行為に該当しないことを確認しました。',
		no_button: 'いいえ',
		apply_button: '申請する',
		// Missing keys for Top.jsx
		draftSubmittedSuccessfully: 'ドラフトが正常に提出されました！',
		noDraftToSubmit: '提出するドラフトがありません',
		errorSubmittingDraft: 'ドラフトの提出中にエラーが発生しました',
		noDataFound: 'データが見つかりません',
		addHobby: '趣味を追加',
		addSpecialSkill: '特技を追加',
		addDescription: '説明を追加',
		addTag: 'タグを追加',
		hobbiesDetailDescription: '趣味についての詳細説明',
		specialSkillsDetailDescription: '特技についての詳細説明',
		hobbiesTagPlaceholder: '趣味タグを入力してください',
		specialSkillsTagPlaceholder: '特技タグを入力してください',
		hobbiesDescriptionPlaceholder: '趣味について詳しく説明してください',
		specialSkillsDescriptionPlaceholder: '特技について詳しく説明してください',
		hobbiesTags: '趣味タグ',
		specialSkillsTags: '特技タグ',
		// Missing keys for ChekProfile.jsx
		statusUpdatedSuccessfully: 'ステータスが正常に更新されました',
		errorUpdatingStatus: 'ステータスの更新中にエラーが発生しました',
		// Missing keys for general usage
		search_placeholder: '名前、フリガナ、IDで検索...',
		search_filters: '検索フィルター',
		no_matches_found: '一致するものが見つかりません',
		start_typing: '入力を開始して候補を表示',
		// Missing keys for hardcoded Japanese text
		entrance: '入学',
		graduation: '卒業',
		credits_unit_short: '単位',
		qualifications: '資格',
		student_grades: '学生成績',
		specialized_knowledge: '専門知識',
		personality: '個性',
		work_experience: '実務経験',
		career_goals: 'キャリア目標',
		comments: 'コメント',
		company_introduction_video: '紹介動画',
		student_label: '学生',
		age_label: '年齢',
		age_unit: '歳',
		application_date: '申請日',
		application_count: '申請回数',
		confirmation_status: '確認状況',
		unconfirmed: '未確認',
		confirmed: '確認済',
		company_video_url_placeholder: '例：https://youtube.com/',
		save_button: '保存',
		no_video_set: '動画が設定されていません',
		company_introduction_video_description: '雇用主から提供された企業紹介動画をご覧いただけます',
		new_target_audience_add: '新しい対象者を追加',
		required_skills_placeholder: '例: HTML/CSS/JavaScriptの実務経験（3年以上）',
		preferred_skills_placeholder: '例: TypeScriptの実務経験',
		// CompanyProfile page translations
		business_content_placeholder: '事業内容',
		recruitment_requirements: '募集要件',
		target_person: '対象者',
		target_audience_placeholder: '対象者',
		additional_info: '追加情報',
		work_location_placeholder: '勤務地',
		work_hours: '勤務時間',
		work_hours_placeholder: '勤務時間',
		salary_placeholder: '給与',
		benefits_placeholder: '社会保険',
		allowances_placeholder: 'その他手当・福利厚生',
		selection_process_placeholder: '採用までの流れ',
		established: '設立',
		established_placeholder: '設立日',
		employee_count: '従業員数',
		employee_count_placeholder: '従業員数',
		location_placeholder: '所在地',
		company_description_placeholder: '企業の説明を',
		saving: '保存中...',
		company_not_assigned_title: '会社情報が見つかりません',
		company_not_assigned_desc: 'まだどの会社にも割り当てられていません。管理者にお問い合わせください。',
		errorFetchingData: 'データの読み込みに失敗しました。もう一度お試しください。',
		// Language settings translations
		unsaved_changes_title: '未保存の変更',
		language_change_unsaved_message: '未保存の変更があります。言語を変更する前に保存しますか？',
		save_and_continue: '保存して続行',
		discard_changes: '変更を破棄',
		display_language: '表示言語',
		language_settings: '言語設定',
		language_change_notice: 'アプリケーションの表示言語を変更します',
		// CreateSkill page translations
		it_skills_manager: 'スキルマネージャー',
		search_skills: 'スキルを検索...',
		add_new_skill: '新しいスキルを追加',
		skills: 'スキル',
		no_skills_found: 'スキルが見つかりません',
		no_skills_available: '利用可能なスキルはありません',
		try_adjusting_search: '検索条件を調整してみてください',
		start_adding_skill: '最初のITスキルを追加してください',
		edit_skill: 'スキルを編集',
		skill_name: 'スキル名',
		choose_color: '色を選択',
		color_code: 'カラーコード',
		update: '更新',
		create: '作成',
		confirm_delete: '削除の確認',
		delete_skill_confirmation: 'このスキルを削除してもよろしいですか？この操作は元に戻せません。',
		failed_fetch_skills: 'スキルの取得に失敗しました',
		skill_name_required: 'スキル名は必須です',
		skill_updated_success: 'スキルが正常に更新されました',
		skill_created_success: 'スキルが正常に作成されました',
		failed_save_skill: 'スキルの保存に失敗しました',
		skill_deleted_success: 'スキルが正常に削除されました',
		failed_delete_skill: 'スキルの削除に失敗しました',
		// Recruiter file translations
		company_documents: '資料',
		upload_info: '最大3ファイル（PDF、Word、Excel、PowerPoint）をアップロードできます。合計最大サイズ：20MB',
		select_files: 'ファイルを選択',
		selected_files: '選択されたファイル',
		uploading: 'アップロード中...',
		upload: 'アップロード',
		max_files_info: 'ファイル数の上限（３ファイル）に達しました。新しいファイルをアップロードするには、既存のファイルを削除してください。',
		max_files_reached: '最大ファイル数に達しました（3ファイル）',
		can_upload_only: 'アップロード可能：',
		more_files: 'ファイル',
		invalid_file_type: '無効なファイルタイプ',
		insufficient_space: '容量不足。利用可能',
		available: '利用可能',
		space_used: '使用済み容量',
		file_too_large: 'ファイルが大きすぎます',
		total_size_exceeded: '合計サイズが20MBを超えました',
		files_uploaded_successfully: 'ファイルが正常にアップロードされました',
		upload_failed: 'アップロードに失敗しました',
		no_files_uploaded: '資料が設定されていません',
		no_intro_pages_set: 'ページが設定されていません',
		download_failed: 'ダウンロードに失敗しました',
		delete_file: 'ファイルを削除',
		delete_file_confirm: 'このファイルを削除してもよろしいですか？',
		deleting: '削除中...',
		file_deleted: 'ファイルが正常に削除されました',
		delete_failed: '削除に失敗しました',
		// Recruiter profile (override/add ensure available in JA)
		addNews: 'ニュース追加',
		noNewsFound: 'ニュースが見つかりません',
		noNewsAvailable: 'ニュースはありません',
		tryAdjusting: '検索条件を調整してみてください',
		noImageAvailable: '画像がありません',
		confirmDeleteNews: 'このニュースを削除してもよろしいですか？',
		hashtagPlaceholder: '例: テクノロジー, ニュース, 更新',
		updateNews: '更新',
		filter: 'フィルター',
		clear: 'クリア',
		apply: '決定',
		noSkills: '利用可能なスキルはありません。',
		filter_all: 'すべて',
		filter_required: '必須',
		filter_optional: '任意',
		sort_required_first: '必須を優先',
		add_required: '必須追加',
		add_optional: '任意追加',
		login_failed: 'ログインに失敗しました。メールアドレスとパスワードを確認してください。',
		no_data_found: 'データが見つかりませんでした',
		next: '次へ',
		expectedGraduationYearMonth: '卒業予定年（月）',
		number_of_applications: '申請回数',
		changed_elements: '変更項目',
		education: '学歴',
		add_arubaito: 'アルバイトを追加',
		arubaito: 'アルバイト',
		no_arubaito: 'アルバイトがまだ追加されていません。',
		licenses: '資格・免許',
		no_licenses: '資格・免許がまだ追加されていません。',
		add_license: '資格・免許を追加',
		address_furigana: '出身地 (フリガナ)',
		additional_info: '追加情報',
		additional_address: '追加住所',
		additional_address_furigana: '追加住所（フリガナ）',
		additional_email: '追加メール',
		additional_phone: '追加電話',
		additional_index: '追加インデックス',
		isMarried: '配偶者の有無',
		married: '既婚',
		single: '未婚',
		postal_code: '郵便番号',
		no_education: '学歴がまだありません。',
		no_work_experience: 'まだ職務経歴がありません。',
		edit_education: '学歴を編集',
		add_education: '学歴を追加',
		edit_work_experience: '職務経歴を編集',
		add_work_experience: '職務経歴を追加',
		present: '現在',
		company: '会社名',
		role_position: '役職',
		description_label: '説明',
		from_yyyy_mm: '開始 (YYYY-MM)',
		to_yyyy_mm: '終了 (YYYY-MM)',
		work_experience_from_placeholder: '2023-01',
		work_experience_to_placeholder: '2024-01 または空欄',
		institution: '学校名',
		status: '状況',
		year: '年',
		month: '月',
		edit_arubaito: 'アルバイトを編集',
		add_arubaito_form: 'アルバイトを追加',
		edit_license: '資格・免許を編集',
		add_license_form: '資格・免許を追加',
		certificate_name: '資格・免許名',
		loading: '読み込み中...',
		institution_placeholder: '例：東京大学',
		status_placeholder: '例：卒業、在学中',
		year_placeholder: '2023',
		month_placeholder: '03（任意）',
		certificate_placeholder: '例：TOEIC 800、運転免許証',
		company_placeholder: '例：コンビニエンスストア',
		role_placeholder: '例：レジ担当',
		period_placeholder: '例：2023-01～2024-01',
		period: '期間',
		download_cv: '履歴書をダウンロード',
		error_loading_data: 'データの読み込みに失敗しました',
		error_fetching_draft: '下書きデータの取得に失敗しました',
		unable_to_determine_student_id: '学生IDを特定できません',
		loading_student_profile: 'プロフィールを読み込み中...',
		loading_questions: '質問を読み込み中...',
		changes_cancelled: '変更をキャンセルしました',
		item_deleted_success: '削除しました',
		error_deleting_item: '削除に失敗しました。もう一度お試しください。',
		error_label: 'エラー',
		no_valid_student_id: '有効な学生IDが見つかりません',
		invalid_video_urls_removed: '無効な動画URLが削除されました。',
		max_three_videos: '最大3件まで追加できます',
		upload_failed_file_invalid: 'ファイルのアップロードに失敗しました。ファイルサイズやフォーマットを確認してください。',
		required: '必須',
		add_role: '役割を追加',
		url_placeholder: 'https://example.com',
		url_placeholder_page: 'https://example.com/page',
		credit_details_disabled: '単位詳細ページは現在無効です',
		credit_details_load_error: '単位詳細を読み込めませんでした',
		credit_details_sync_success: '単位詳細の同期に成功しました',
		credit_details_sync_failed: '単位詳細の同期に失敗しました',
		file_size_max_5mb: 'ファイルが大きすぎます。最大5MBまでです。',
		invalid_file_type_images_only: '無効なファイル形式です。画像ファイルのみ選択してください。',
		video_thumbnail: '動画サムネイル',
		company_intro_video: '会社紹介動画',
		video_thumbnails: '動画サムネイル',
		close: '閉じる',
		close_filter_modal: 'フィルターモーダルを閉じる',
		more: 'その他',
		delete_aria: '削除',
		download_aria: 'ダウンロード',
		enrollment: '入学',
		graduation: '卒業',
		jdu: 'JDU',
		// Mail Service
		ms_title: 'メールサービス',
		ms_periodic_email: '定期メール',
		ms_inactive_student_email: '未更新学生メール',
		ms_periodic_email_settings: '定期メール設定',
		ms_send_period: '送信間隔',
		ms_search_period: '検索期間',
		ms_email_subject: 'メール件名',
		ms_email_body: 'メール本文',
		ms_plain_text_hint: 'プレーンテキストで入力してください。改行は保持されます。',
		ms_inactive_description: '差し戻し後、選択した週間間隔内に再提出していない学生を検索します。各間隔は排他的です（例：1週間＝7〜13日前のみ）。',
		ms_inactive_period_title: '差し戻し学生（週間別）',
		ms_1_week: '1週間',
		ms_2_weeks: '2週間',
		ms_3_weeks: '3週間',
		ms_4_weeks: '4週間',
		ms_1_month: '1ヶ月',
		ms_3_months: '3ヶ月',
		ms_6_months: '6ヶ月',
		ms_9_months: '9ヶ月',
		ms_1_year: '1年',
		ms_1_year_plus: '1年以上',
		ms_6_months_plus: '6ヶ月以上',
		ms_save: '保存',
		ms_search: '検索',
		ms_reset: 'リセット',
		ms_send_email: 'メール送信',
		ms_students_found: '該当学生数',
		ms_no_students_found: 'この期間に該当する学生はいません',
		ms_student_id: '学籍番号',
		ms_student_name: '氏名',
		ms_email: 'メール',
		ms_last_updated: '最終更新日',
		ms_last_updated_by: '最終更新者',
		ms_last_activity: '最終活動日',
		ms_never_active_title: '未活動の学生',
		ms_never_active_description: '登録以来、一度もドラフトを提出していない学生を検索します。',
		ms_registered_at: '登録日',
		ms_never_active_students: '未活動の学生',
		ms_never_active_hint: '（登録以来、一度もドラフトを提出していません）',
		ms_setting_updated: '設定が更新されました',
		ms_error_loading: '設定の読み込みに失敗しました',
		ms_error_updating: '設定の更新に失敗しました',
		ms_error_searching: '学生の検索に失敗しました',
		ms_error_sending: 'メール送信に失敗しました',
		ms_fill_all_fields: 'すべてのフィールドを入力してください',
		ms_emails_sent: '送信完了',
		ms_send_result: '送信結果',
		ms_successful: '成功',
		ms_failed: '失敗',
		ms_year: '年',
		ms_month: 'ヶ月',
		ms_week: '週間',
		ms_day: '日',
		ms_days: '日間',
		ms_schedule_day_of_month: '送信日（毎月）',
		ms_schedule_hour: '送信時刻',
		ms_schedule_day_of_month_hint: '毎月の送信日（1〜28）',
		ms_schedule_hour_hint: '送信時刻（0〜23）',
		// Sidebar & profile header
		maintenance: 'メンテナンス',
		expected_graduation_month: 'JDU卒業予定年月',
		enrolled_partner_university: '在籍提携大学',
		not_set: '未設定',
		getLink: 'リンクを取得',
		generating: '生成中...',
		regenerateLink: '新しいリンク',
		linkExpiresIn: '残り有効期限',
		link_expired: 'リンクの有効期限切れ',
		link_expiry_notice: 'リンクの有効期限',
		link_generated_success: 'リンクが正常に生成されました！',
		link_generation_failed: 'リンクの生成に失敗しました',
		link_copied: 'リンクをコピーしました！',
		linkCopied: 'リンクがコピーされました！',
		profile_not_public: 'まずプロフィールを公開してください',
		deactivate_link: 'リンクを無効化',
		deactivating: '無効化中...',
		link_deactivated_success: 'リンクを無効化しました',
		link_deactivate_failed: 'リンクの無効化に失敗しました',
		confirm_to_download_cv: '読みました',
		resumeDownloadWarning: `⚠️ 履歴書をダウンロードする前に必ずお読みください！
学生の皆様へ この履歴書（CV）は、ウェブサイトのポートフォリオの情報をもとにシステムが自動で作成したものです。そのため、内容が100％正しいとは限りません。ダウンロードした後は、必ず**「見本 履歴書」**と見比べて、自分で内容をチェックしてください！
❗️ 多くの人が間違えやすい、特に注意すべきポイント：
1) 「職務経歴書」について: 最初の表の部分には、大学の「Cowork（コワーク）」で参加したプロジェクトだけを書きます。自動でデータが入力されるため、他の情報と混ざっている可能性があります。お願いですから、「Cowork」のプロジェクトと、外での経験（アルバイト、フリーランス、他の会社のプロジェクトなど）はしっかり分けて、それぞれ正しい場所に書かれているか確認してください。
2) 住所 – 「現住所」と「連絡先」:
「現住所」: 今、あなたが実際に住んでいる場所の住所を書きます。
「連絡先」: あなたの保護者（ご両親など）が住んでいる住所を書きます。 もし両方が同じ場所の場合は、下の部分（連絡先）には*「同上」*とだけ書いてください。 正しく入力されているか確認してください。
3) 「自己PR」: この部分は、自動で作られたテンプレートのままにしてはいけません。提出する会社や状況に合わせて、毎回自分で書き直す必要があります。自分の言葉で正しく修正されているか確認してください。
💡 もし履歴書の修正で分からないことや問題があれば、遠慮なく先輩や大学のスタッフ（職員）に相談してください。`,

		companies: '企業',
		add_recruiter: '採用担当者を追加',
		choose_existing_company: '既存の企業を選択',
		create_new_company: '新しい企業を作成',
		confirm_delete_recruiter: 'この採用担当者を削除してもよろしいですか？',
		not_assign_company: 'この採用担当者はまだ企業に割り当てられていません。',
		companies: '企業',
		new_company: '新しい企業',
		company_name: '企業名',
		company_name_placeholder: '企業名、住所、採用担当者名で検索...',
		is_partner_true: '採用企業',
		is_partner_false: '代理店',
		company_address: '住所',
		recruiters: '採用担当者',
		no_companies_found: '企業が見つかりません',
		assign_recruiter: '採用担当者を割り当てる',

		search_recruiters: '採用担当者を検索...',
		is_partner: '提携企業',
		partner_chip: '提携企業',
		set_parent_recruiter: 'メインの採用担当者として設定しました',
		removed_parent_recruiter: 'メインの採用担当者を解除しました',
		create_company: '企業を作成',
		company_identity: '企業識別情報',
		no_recruiters_found: '採用担当者が見つかりません',
		select_company: '企業を選択',
		assigned_recruiter_to_company: '採用担当者を企業に割り当てました',
		unassigned_recruiter_from_company: '採用担当者を企業から削除しました',
		updated_company_identity: '企業識別情報を更新されました',
		updated_company_profile: '企業プロフィールを更新されました',
		details: '詳細',
		edit_recruiter: '採用担当者を編集',
		company_change_hint: '会社は会社ページからのみ変更できます。',
	},
	uz: {
		age: 'Yosh',
		tagline: 'Bir jumlalik PR',
		company_information: "Kompaniya ma'lumotlari",
		company_name: 'Kompaniya nomi',
		company_Address: 'Manzil',
		company_website: 'Veb-sayt',
		company_description: 'Kompaniya haqida',
		company_overview: 'Kompaniya haqida',
		business_overview: "Faoliyat yo'nalishlari",
		employee_Count: 'Xodimlar soni',
		established_Date: 'Tashkil topgan sana',
		company_capital: 'Ustav kapitali',
		company_revenue: 'Daromad (sotuvlar)',
		company_representative: 'Vakil',

		job_details: "Bo'sh ishlar sharhi",
		job_title: 'Qidirilayotgan lavozim',
		job_description: 'Vazifalar tavsifi',
		japanese_level: 'Yapon tili darajasi',
		application_requirements_other: 'Boshqa talablar',
		required_skills: "Majburiy: tajriba/ko'nikmalar",
		welcome_skills: "Afzallik: tajriba/ko'nikmalar",
		recommended_skills: "Tavsiya: tajriba/ko'nikmalar",
		recommended_licenses: 'Tavsiya: litsenziya/sertifikatlar',
		recommended_other: 'Tavsiya: boshqalar',
		number_of_openings: "Bo'sh ish o'rinlari soni",
		employment_type: 'Bandlik turi',
		probation_period: 'Sinov muddati',
		employment_period: 'Ish davri',
		work_location: 'Ish joyi',

		// Tanishuv sahifasi bo'limi
		intro_page: 'Tanishuv sahifasi',
		intro_page_thumbnail: 'Tanishuv sahifasi uchun rasm',
		intro_page_links: 'Tanishuv sahifasi havolalari',
		newIntroLinkTitle: 'Havola nomi',
		newIntroLinkUrl: 'Havola URL',
		intro_link_title: 'Havola nomi',
		max_four_links: "Ko'pi bilan 4 ta havola qo'shish mumkin",

		// Kadrlar opsiyalari
		retirement_benefit: "Ishdan bo'shash nafaqasi",
		telework_availability: 'Masofadan ishlash mavjudligi',
		housing_availability: 'Yotoqxona/uy-joy mavjudligi',
		relocation_support: "Aviachiptalar/ko'chish xarajatlarini qoplash",
		airport_pickup: 'Kelganda aeroportdan kutib olish',

		compensation_benefits: 'Ish haqi va imtiyozlar',
		salary: 'Ish haqi',
		salary_increase: 'Maosh oshirilishi',
		bonus: 'Bonus',
		allowances: "Qo'shimcha to'lovlar",
		holidays_vacation: "Dam olish / ta'til",
		benefits: "Ijtimoiy sug'urta",

		other: 'Boshqa',
		other_notes: 'Boshqa',

		selection_info: 'Tanlov jarayoni',
		selection_process: 'Ishga qabul jarayoni',
		target_audience: 'Maqsad',
		interview_method: 'Suhbat turi',
		apply: 'tasdiqlash',
		rank: "o'rin",
		title: 'Sarlavha',
		description: 'Tavsif',
		sourseLink: 'Manba havolasi',
		hashtag: 'Xeshteg',
		CreateNews: 'Yengilik yaratish',
		EditNews: "Yangilikni o'zgartirish",
		visibleToRecruiter: "Ish beruvchiga ko'rinadigan",
		newsVisibility: "Yangilik ko'rinishi",
		universityNews: 'Universitet yangiliklari',
		universityNewsDescription: "Hammaga ko'rinadi (xodimlar, adminlar, talabalar, ish beruvchilar)",
		recruiterNews: 'Ish beruvchi yangiliklari',
		recruiterNewsDescription: "Ish beruvchilardan tashqari hammaga ko'rinadi",
		imageRequired: 'Rasm majburiy',
		descriptionRequired: 'Tavsif majburiy',
		fillRequiredFields: "Iltimos, barcha majburiy maydonlarni to'ldiring (Sarlavha, Tavsif, Rasm)",
		cencel: 'Bekor qilish',
		searchByHashtag: "Xeshteg, kompaniya nomi yoki yangilik sarlavhasi bo'yicha qidiring...",
		checkOut: "Korxonalardan eng so'nggi e'lonlar va takliflarni ko'rib chiqing",
		newsHighlights: 'Yangiliklar',
		news: 'Korxona yangiliklari',
		Qualifications: 'Sertifikatlar',
		studetentSkillManagment: "Talaba ko'nikmalarini boshqarish",
		studentCredits: 'Talabaning kreditlar soni',
		major: 'Mutaxassislik',
		origin: 'Kelib chiqish',
		jobType: 'Istagan ish turi',
		qualification: 'Sertifikat',
		japaneseSpeechContest: "Yapon tili notiqlik san'ati",
		itContest: 'IT tanlovi',
		languageSkills: "Til ko'nikmalari",
		japanese: '日本語',
		general: 'Umumiy',
		home: 'Bosh sahifa',
		profile: 'Profil',
		student_search: 'Talabalarni qidirish',
		student_check: 'Talabalarni tekshirish',
		staff: 'Xodimlar',
		recruiter: 'Ish beruvchi',
		bookmarked: 'Saqlanganlar',
		settings: 'Sozlamalar',
		help: 'Yordam',
		student_qa: 'Talaba Savollari',
		logout: 'Chiqish',
		Are_you_sure: 'Chiqmoqchi ekanligingizga ishonchingiz komilmi?',
		yesModal: 'Ha',
		noModal: "Yo'q",
		jdu_portfolio: 'JDU Portfolio',
		japan: 'Yaponiya',
		uzbekistan: "O'zbekiston",
		edit: 'Tahrirlash',
		next_button: 'Talabalarni qidirish ➜',
		large_class_photo_alt: 'Katta sinf rasmi',
		group_photo_alt: 'Guruh rasmi',
		changes_saved: "O'zgartirishlar muvaffaqiyatli saqlandi!",
		grade: "o'quv yili",
		grade1: '1-kurs',
		grade2: '2-kurs',
		grade3: '3-kurs',
		grade4: '4-kurs',
		programming_languages: 'Dasturlash tillari',
		any: 'Kamida bittasi',
		all: 'Hammasi',
		search_items: 'Qidirish...',
		select_all_filtered: 'Hammasini tanlash (filtrl.)',
		clear_filtered: 'Tozalash (filtrl.)',
		selected: 'Tanlangan',
		select_all: 'Hammasini tanlash',
		clear_all: 'Hammasini bekor qilish',
		jlpt: 'Yapon tili daraja testi',
		ielts: 'IELTS',
		jdu_certification: 'JDU Yapon tili sertifikati',
		credits: 'Kreditlar',
		credits_unit: ' Kreditlar ichida',
		partner_university: 'Hamkor universitet',
		special_skills: "Maxsus ko'nikmalar",
		yes: 'Ha',
		no: "Yo'q",
		tokyo_communication_university: 'Tokio aloqa universiteti',
		kyoto_tachibana_university: 'Kioto Tachibana universiteti',
		sanno_university: 'Sanno universiteti',
		sanno_junior_college: 'Sanno qisqa muddatli kollej',
		niigata_sangyo_university: 'Niigata sanoat universiteti',
		otemae_university: 'Otemae universiteti',
		okayama_university_of_science: 'Okayama fan-texnika universiteti',
		unaffiliated: 'Biriktirilmagan',
		student: 'Talaba',
		rows_per_page: 'Qatorlar soni:',
		search_by_name: "Ism bo'yicha qidirish",
		search: 'Qidiruv',
		reset: 'Bekor qilish',
		name_search: 'Ism orqali qidiruv',
		department: "Bo'lim",
		position: 'Lavozim',
		phone_number: 'Telefon raqami',
		name: 'Ism',
		saveDraft: 'Yangi Qoralama',
		updateDraft: 'Ustiga yozish',
		submitAgree: 'Topshirish/Rozi',
		sync: 'Sinxronizatsiya',
		user_avatar: 'Foydalanuvchi rasmi',
		company_logo: 'Kompaniya logosi',
		upload_picture: 'Rasm yuklash',
		user: 'Foydalanuvchi',
		first_name: 'Ism',
		first_name_furigana: 'Ism (Furigana)',
		last_name: 'Familiya',
		last_name_furigana: 'Familiya (Furigana)',
		furigana_help: 'Talaffuzni hiragana/katakana da kiriting',
		phone: 'Telefon',
		email: 'Elektron pochta',
		change_password: "Parolni o'zgartirish",
		current_password: 'Parol',
		new_password: 'Yangi parol',
		confirm_password: 'Parolni tasdiqlash',
		toggle_password_visibility: "Parolni ko'rishni yoqish/o'chirish",
		contact_info: "Kontakt ma'lumotlari",
		contact_email: 'Kontakt email',
		contact_phone: 'Kontakt telefon',
		working_hours: 'Ish vaqti',
		location: 'Joylashuv',
		personal_info: "Shaxsiy ma'lumotlar",
		user_fetch_error: 'Foydalanuvchi maʼlumotlarini olishda xatolik yuz berdi.',
		profile_update_success: 'Profil muvaffaqiyatli yangilandi.',
		profile_update_failed_retry: "Profilni yangilashda xatolik yuz berdi. Qayta urinib ko'ring.",
		unknown_role_error: 'Nomaʼlum rol aniqlangan.',
		password_mismatch: 'Parollar mos kelmadi.',
		current_password_required: 'Joriy parolingizni kiriting.',
		unknown_role: "Noma'lum rol.",
		fetch_user_failed: "Foydalanuvchi ma'lumotlarini olish muvaffaqiyatsiz tugadi.",
		profile_updated: 'Profil muvaffaqiyatli yangilandi.',
		profile_update_failed: "Profilni yangilashda xatolik yuz berdi. Qaytadan urinib ko'ring.",
		sync_successful: "Sinxronizatsiya muvaffaqiyatli o'tdi.",
		sync_failed: "Sinxronizatsiya muvaffaqiyatsiz tugadi. Qaytadan urinib ko'ring.",
		notFoundTitle: '404 - Sahifa topilmadi',
		notFoundMessage: 'Qidirgan sahifangiz mavjud emas.',
		backToHome: 'Bosh sahifaga qaytish',
		maintenanceTitle: 'Texnik xizmat korsatish rejimida',
		checkingMaintenance: 'Texnik xizmat holatini tekshiryapmiz...',
		checkingStatus: 'Avtomatik ravishda holatni tekshiryapmiz...',
		// Maintenance page translations
		maintenance_announcement: "Texnik xizmat e'lonnomasi",
		announcement_settings: "E'lon sozlamalari",
		message: 'Xabar',
		enter_announcement_message: "E'lon xabarini kiriting...",
		visibility_settings: "Ko'rinish sozlamalari",
		make_public: "E'lonni kirish sahifasida barcha foydalanuvchilar uchun ko'rsatish",
		announcement_visible_to_all_users: "Bu e'lon hozirda kirish sahifasida barcha foydalanuvchilar uchun ko'rsatilmoqda.",
		announcement_hidden_from_users: "Bu e'lon hozirda foydalanuvchilar uchun yashiringan.",
		created_at: 'Yaratilgan',
		last_updated: 'Oxirgi yangilanish',
		message_required: 'Xabar talab qilinadi',
		announcement_updated: "E'lon muvaffaqiyatli yangilandi",
		error_updating_announcement: "E'lonni yangilashda xatolik",
		please_update_announcement_first: "Iltimos, avval e'lonni yangilang",
		announcement_made_public: "E'lon endi barcha foydalanuvchilar uchun ko'rinadi",
		announcement_made_private: "E'lon endi foydalanuvchilar uchun yashiringan",
		error_toggling_status: "Holatni o'zgartirishda xatolik",
		unauthorizedTitle: "403 - Ruxsat yo'q",
		unauthorizedMessage: 'Siz ushbu sahifaga kirish huquqiga ega emassiz.',
		unauthorizedButton: 'Kirish sahifasiga qaytish',
		back: 'Orqaga',
		add: "yangi qo'shish",
		edit_profile: 'Profilni tahrirlash',
		liveProfile: 'Jonli profil',
		draftProfile: 'Qoralama profil',
		qa_edit: 'QA tahrirlash',
		faq_heading: 'Tez-tez beriladigan savollar',
		faq_no_answer: 'Javob yoʻq',
		q_edit: 'Savolni tahrirlash',
		age: 'Yosh',
		student_id: 'Talaba ID',
		top: 'Asosiy',
		qa: 'Q&A',
		stats: "Kreditlar va Ko'nikmalar",
		errorFetchingStudent: "Talaba ma'lumotlarini olishda xatolik yuz berdi.",
		changesSavedSuccessfully: "O'zgarishlar muvaffaqiyatli saqlandi!",
		errorSavingChanges: "O'zgarishlarni saqlashda xatolik yuz berdi.",
		errorUploadingFile: 'Faylni yuklashda xatolik yuz berdi.',
		savingChanges: "O'zgarishlar saqlanmoqda...",
		changesSaved: "O'zgarishlar saqlandi",
		dataRecovered: "Ma'lumotlar muvaffaqiyatli tiklandi",
		recoverUnsavedChanges: "Saqlanmagan o'zgarishlarni tiklaysizmi?",
		unsavedChangesFound: "Oldingi tahrir sessiyangizdan saqlanmagan o'zgarishlar topildi. Ularni tiklashni xohlaysizmi?",
		lastModified: "Oxirgi o'zgarish",
		discard: 'Bekor qilish',
		restore: 'Tiklash',
		unsavedChangesTitle: "Saqlanmagan o'zgarishlar",
		unsavedChangesMessage: "Saqlanmagan o'zgarishlaringiz bor. Ularni bekor qilishni xohlaysizmi?",
		continueEditing: 'Tahrirlashni davom ettirish',
		discardChanges: "O'zgarishlarni bekor qilish",
		unsavedChangesLanguageTitle: "Tilni o'zgartirishdan oldin o'zgarishlarni saqlaysizmi?",
		unsavedChangesLanguageMessage: "Saqlanmagan o'zgarishlaringiz bor. Tilni o'zgartirishdan oldin ularni saqlashni xohlaysizmi?",
		discardAndSwitch: "Bekor qilish va o'zgartirish",
		saveAndSwitch: "Saqlash va o'zgartirish",
		dataRestoredAfterLanguageSwitch: "Til o'zgartirilgandan so'ng ma'lumotlaringiz tiklandi",
		unsavedChangesNavigationTitle: "Sahifadan chiqishdan oldin o'zgarishlarni saqlaysizmi?",
		unsavedChangesNavigationMessage: "Saqlanmagan o'zgarishlaringiz bor. Bu sahifadan chiqishdan oldin ularni saqlashni xohlaysizmi?",
		discardAndLeave: 'Bekor qilish va chiqish',
		saveAndLeave: 'Saqlash va chiqish',
		fileUploadSuccess: 'Fayl muvaffaqiyatli yuklandi.',
		selfIntroduction: "O'zini tanishtirish",
		deliverables: 'Natijalar',
		hobbies: 'Qiziqishlar',
		specialSkills: "Maxsus ko'nikmalar",
		itSkills: "IT Ko'nikmalar",
		otherSkills: 'Boshqa sertifikat/litsenziya/mukofotlar',
		// Deliverables translations
		addDeliverable: "Mahsulot qo'shish",
		createDeliverable: 'Mahsulot yaratish',
		editDeliverable: 'Mahsulotni tahrirlash',
		noDeliverables: "Hali mahsulotlar yo'q",
		addFirstDeliverable: "Birinchi mahsulotingizni qo'shing",
		deliverableCreated: 'Mahsulot muvaffaqiyatli yaratildi!',
		deliverableUpdated: 'Mahsulot muvaffaqiyatli yangilandi!',
		deliverableDeleted: "Mahsulot muvaffaqiyatli o'chirildi!",
		titleRequired: 'Sarlavha talab qilinadi',
		filesRequired: 'Kamida bitta rasm talab qilinadi',
		selectImages: 'Rasmlarni tanlang',
		addMoreImages: "Yana rasm qo'shish",
		replaceImages: 'Rasmlarni almashtiring',
		currentImages: 'Joriy rasmlar',
		newImages: 'Yangi rasmlar',
		images: 'rasmlar',
		view: "Ko'rish",
		openLink: 'Havolani ochish',
		openCodeLink: "Kodini ko'rish",
		allImages: 'Barcha rasmlar',
		confirmDelete: "Bu mahsulotni o'chirishga ishonchingiz komilmi?",
		上級: "Ilg'or",
		中級: "O'rta",
		初級: "Boshlang'ich",
		threeYearsOrMore: "Uch yil yoki undan ko'p",
		oneToOneAndHalfYears: 'Bir yildan bir yarim yilgacha',
		lessThanThreeYears: 'Uch yildan kam',
		basic: 'Asosiy',
		loading: 'Yuklanmoqda...',
		save: 'Saqlash',
		cancel: 'Bekor qilish',
		editProfile: 'Profilni tahrirlash',
		skillExists: "Bu darajada bu ko'nikma allaqachon mavjud!",
		skillAdded: "Ko'nikma muvaffaqiyatli qo'shildi!",
		skillDeleted: "Ko'nikma o'chirildi!",
		selectSkill: "Ko'nikmani tanlang",
		skill: 'Skill',
		level: 'Daraja',
		languageName: 'Til nomi',
		awardName: 'Mukofot/Sertifikat nomi',
		noLanguageSkillsEdit: "Hali qo'shilmagan",
		noSkillsEdit: "Hali ko'nikmalar qo'shilmagan. Yuqoridagi formadan qo'shing.",
		noLanguageSkills: "Ko'nikmalar yo'q",
		noAwardsEdit: "Hali qo'shilmagan",
		noAwards: "Mukofotlar yo'q",
		levelInfo: "Yuqori: 3 yildan ortiq | O'rtacha: 3 yildan kam | Boshlang'ich: 1 yildan kam",
		notEntered: 'Kiritilmagan',
		not_submitted: 'Toplanmagan',
		changed: "O'zgargan",
		// ChangedFieldsModal
		changed_elements_list: "O'zgartirilgan maydonlar ro'yxati",
		changes_count_message: "Jami %d ta o'zgarish",
		changes_count_short: "%d ta o'zgarish",
		no_changes: "O'zgarish yo'q",
		category_basic_info: "Asosiy ma'lumot",
		category_skills_abilities: "Ko'nikmalar",
		category_interests: 'Qiziqishlar',
		category_certifications: 'Sertifikatlar',
		category_portfolio: 'Portfolio',
		category_qa: 'S&J',
		category_other: 'Boshqa',
		gallery: 'Galereya',
		hobbies_description: 'Qiziqishlar tavsifi',
		special_skills_description: 'Maxorat tavsifi',
		none: "Yo'q",
		showMore: "Ko'proq ko'rish",
		showLess: 'Yopish',
		notifications: 'Bildirishnomalar',
		mark_all_read: "Barchasini o'qish",
		mark_as_read: "O'qilgan deb belgilash",
		readed: "O'qilgan",
		unread: "O'qilmagan",
		read: "O'qilgan",
		expand: 'Kengaytirish',
		collapse: "Yig'ish",
		no_notifications: "Hozircha xabarnomalar yo'q...",
		no_unread_notifications: "Hozircha o'qilmagan xabarnomalar yo'q...",
		no_read_notifications: "Hozircha o'qilgan xabarnomalar yo'q...",
		new: 'YANGI',
		ok: 'OK',
		delete: "O'chirish",
		warning: 'Ogohlantirish',
		confirmApprove: 'Tasdiqlaysizmi?',
		confirmSendBack: 'Qaytarasizmi?',
		confirmApproveDesc: 'Bu amal talabaga "tasdiqlandi" xabarini yuboradi. Davom etishni xohlaysizmi?',
		confirmSendBackDesc: 'Bu amal talabaga izoh bilan qaytarish xabarini yuboradi. Davom etishni xohlaysizmi?',
		staffFeedbackTitle: 'Xodim fikri',
		staffFeedbackHint: 'Iltimos, ushbu fikr asosida profilingizni yangilang va qayta yuboring.',
		pastStaffComments: "O'tgan xodim izohlari",
		staffComment: 'Izoh',
		resubmissionComment: 'Qaytarish izohi',
		approve: 'Tasdiqlash',
		disapprove: 'Tasdiqlamaslik',
		sendBack: 'Qaytarish',
		originPlaceholder: 'Uzbekistan',
		addressFuriganaPlaceholder: 'ウズファイスト',
		studentNotApprovedByStaff: 'Talaba xodimlar tomonidan tasdiqlanishi kerak',
		profileVisibilityEnabled: "Profil ko'rinishi muvaffaqiyatli yoqildi",
		profileHidden: 'Profil muvaffaqiyatli yashirildi',
		errorSettingVisibility: "Ko'rinishni o'rnatishda xatolik",
		navigationWarningTitle: "Saqlanmagan o'zgarishlar",
		navigationWarningMessage: "Sizda saqlanmagan o'zgarishlar bor. Ularni bekor qilib boshqa sahifaga o'tmoqchimisiz?",
		levels: {
			上級: 'Yuqori daraja: 3 yildan ortiq',
			中級: "O'rta daraja: 3 yildan kam",
			初級: "Boshlang'ich daraja: 1 yildan kam",
			JLPT: 'JLPT',
			JDU_JLPT: 'JDU Yapon tili sertifikati',
			IELTS: 'IELTS',
			日本語弁論大会学内: "Yapon tili notiqlik san'ati",
			ITコンテスト学内: 'IT tanlov',
		},
		approved: 'Tasdiqlangan',
		disapproved: 'Rad etildi',
		resubmission_required: 'Qayta topshirish talab etildi',
		submitted: 'Topshirildi',
		draft: 'Qoralama',
		published: 'Ochiq',
		private: 'Maxfiy',
		welcome: 'JDU Portfolio',
		forgotPassword: 'Parolni unutdingizmi',
		resetPassword2: "Parolni tiklash yo'riqnomasini olish uchun elektron pochta manzilingizni kiriting",
		enterYourLogin: 'Elektron pochtangizni kiriting',
		enterYourPassword: 'Parolingizni kiriting',
		mailAddress: 'Elektron pochta manzili',
		send: 'Yuborish',
		loginLabel: 'Kirish',
		profile_publish_request: 'Nashr etish uchun ariza',
		profile_publish_explanation: `Ish qidirishingizni osonlashtirish uchun profilingiz arizangiz asosida JDU Portfolio xizmatida nashr etiladi.
Nashr etish uchun ariza topshirilgandan soʻng, u JDUning Yaponiyada ish bilan taʼminlash boʻlimiga yuboriladi va u erda uning mazmuni xodimlar tomonidan koʻrib chiqiladi.
Agar arizangiz mazmuni boʻyicha kamchiliklar yoki savollar boʻlsa, siz bilan elektron pochta orqali bogʻlanishadi.
Agar arizangiz mazmuni boʻyicha hech qanday kamchiliklar yoki savollar boʻlmasa, arizangiz maʼqullanadi hamda profilingiz eʼlon qilinadi.
Iltimos, profilingiz mazmuni va boshqa maʼlumotlar quyidagi taqiqlangan harakatlarga toʻgʻri kelmasligiga ishonch hosil qiling, Ariza yuborish tugmasini bosib nashr etish uchun ariza bering.`,
		prohibited_actions: '（Taqiqlangan xatti-harakatlar）',
		prohibited_actions_content: `JDU talabalari ushbu xizmatdan foydalanishda quyidagi xatti-harakatlarning amalga oshirilishi taqiqlanadi:
<ol>
<li>Yolg'on yoki noaniq ma'lumot taqdim etish xatti-harakati</li>
<li>Shaxs yoki tashkilotlarni tuhmat qilish, haqorat qilish, tahdid qilish yoki bunga olib kelishi mumkin bo'lgan xatti-harakatlar</li>
<li>Mualliflik huquqi, savdo belgisi va boshqa intellektual mulk huquqlarini o'z ichiga olgan boshqalarning huquqlarini buzish yoki buzish ehtimoli bo'lgan xatti-harakatlar. Masalan: taniqli shaxslar yoki boshqalarning fotosuratlari, asarlari, personajlarini ruxsatsiz foydalanish xatti-harakati</li>
<li>Ushbu xizmatning muammosiz ishlashiga xalaqit berish yoki kompaniya va ish qidiruvchilarning obro'si yoki sha'niga putur yetkazish yoki bunga olib kelishi mumkin bo'lgan xatti-harakatlar. Masalan: ish qidiruvchilarga bevosita murojaat qilish, suhbat o'tkazish kabi xatti-harakatlar</li>
<li>Jinoyat xatti-harakatlari yoki qonunlar yoki jamoat odob-axloq normalariga zid bo'lgan xatti-harakatlar yoki bunga olib kelishi mumkin bo'lgan xatti-harakatlar</li>
<li>Antisotsial kuchlar bilan bog'liq tashkilotlarga mansub bo'lish, antisotsial kuchlarga foyda keltirish yoki ulardan foydalanish kabi nojo'ya munosabatlarga ega bo'lish xatti-harakati yoki bunga olib kelishi mumkin bo'lgan xatti-harakatlar</li>
</ol>`,
		apply_button: 'Yuborish',
		no_button: "Yo'q",
		confirm_no_prohibited_actions: 'Yuqoridagi taqiqlangan xatti-harakatlarni qilmaslikni tasdiqlayman.',
		// Missing keys for Top.jsx
		draftSubmittedSuccessfully: 'Qoralama muvaffaqiyatli topshirildi!',
		noDraftToSubmit: 'Topshirish uchun qoralama mavjud emas',
		errorSubmittingDraft: 'Qoralama topshirishda xatolik yuz berdi',
		noDataFound: "Ma'lumot topilmadi",
		start_checking: 'Tekshirishni boshlash',
		addHobby: "Qiziqish qo'shish",
		addSpecialSkill: "Maxsus ko'nikma qo'shish",
		addDescription: "Tavsif qo'shish",
		addTag: "Teg qo'shish",
		hobbiesDetailDescription: 'Qiziqishlar haqida batafsil tavsif',
		specialSkillsDetailDescription: "Maxsus ko'nikmalar haqida batafsil tavsif",
		hobbiesTagPlaceholder: 'Qiziqish teglarini kiriting',
		specialSkillsTagPlaceholder: "Maxsus ko'nikma teglarini kiriting",
		hobbiesDescriptionPlaceholder: 'Qiziqishlaringiz haqida batafsil yozing',
		specialSkillsDescriptionPlaceholder: "Maxsus ko'nikmalaringiz haqida batafsil yozing",
		hobbiesTags: 'Qiziqish teglari',
		specialSkillsTags: "Maxsus ko'nikma teglari",
		// Missing keys for ChekProfile.jsx
		statusUpdatedSuccessfully: 'Status muvaffaqiyatli yangilandi',
		errorUpdatingStatus: 'Statusni yangilashda xatolik yuz berdi',
		checking: 'Tekshirilmoqda',
		not_approved_yet: 'Hali tasdiqlanmagan',
		approvalStatus: 'Tasdiqlash holati',
		approval_status_unconfirmed: 'Tasdiqlanmagan',
		approval_status_in_review: 'Tekshirilmoqda',
		approval_status_returned: 'Qaytarilgan',
		approval_status_approved: 'Tasdiqlangan',
		visible_status: "Ko'rinish holati",
		visible: "Ko'rinadigan",
		invisible: "Ko'rinmaydigan",
		// Missing keys for general usage
		search_placeholder: 'Qidirish...',
		search_filters: 'Qidiruv filtrlari',
		no_matches_found: 'Mos keladigan narsa topilmadi',
		start_typing: "Takliflarni ko'rish uchun yozishni boshlang",
		// Missing keys for hardcoded text
		entrance: 'Kirish',
		graduation: 'Bitirish',
		credits_unit_short: 'Kreditlar',
		qualifications: 'Malaka',
		student_grades: 'Talaba baholari',
		specialized_knowledge: 'Mutaxassis bilim',
		personality: 'Shaxsiyat',
		work_experience: 'Ish tajribasi',
		career_goals: 'Martaba maqsadlari',
		comments: 'Izohlar',
		company_introduction_video: 'Tanishtiruv video',
		student_label: 'Talaba',
		age_label: 'Yosh',
		age_unit: ' yosh',
		application_date: 'Ariza sanasi',
		application_count: 'Ariza soni',
		confirmation_status: 'Tasdiqlash holati',
		unconfirmed: 'Tasdiqlanmagan',
		confirmed: 'Tasdiqlangan',
		company_video_url_placeholder: 'masalan: https://youtu.be/rSRpRd1E45w?si=3r7PqVgWt67ZA2i5',
		save_button: 'Saqlash',
		no_video_set: "Video o'rnatilmagan",
		company_introduction_video_description: "Ish beruvchi tomonidan taqdim etilgan kompaniya tanishtiruv videosini ko'rishingiz mumkin",
		new_target_audience_add: "Yangi maqsadli auditoriya qo'shish",
		required_skills_placeholder: 'masalan: HTML/CSS/JavaScript bilan amaliy tajriba (3+ yil)',
		preferred_skills_placeholder: 'masalan: TypeScript bilan amaliy tajriba',
		// CompanyProfile page translations
		business_content_placeholder: 'Biznes mazmunini kiriting',
		recruitment_requirements: 'Ishga qabul qilish talablari',
		target_person: 'Maqsadli shaxs',
		target_audience_placeholder: 'Maqsadli auditoriyani kiriting',
		additional_info: 'Qoʻshimcha maʼlumot',
		work_location_placeholder: 'Ish joyini kiriting',
		work_hours: 'Ish vaqti',
		work_hours_placeholder: 'Ish vaqtini kiriting',
		salary_placeholder: 'Maoshni kiriting',
		benefits_placeholder: 'Imtiyozlarni kiriting',
		allowances_placeholder: "Boshqa to'lovlar va imtiyozlarni kiriting",
		selection_process_placeholder: 'Tanlov jarayonini kiriting',
		established: 'Tashkil etilgan',
		established_placeholder: 'Tashkil etilgan sanani kiriting',
		employee_count: 'Xodimlar soni',
		employee_count_placeholder: 'Xodimlar sonini kiriting',
		location_placeholder: 'Manzilni kiriting',
		company_description_placeholder: 'Kompaniya tavsifini kiriting',
		saving: 'Saqlanmoqda...',
		company_not_assigned_title: 'Kompaniya topilmadi',
		company_not_assigned_desc: "Siz hali hech qaysi kompaniyaga bog'liq emassiz. Iltimos, administrator bilan bog'laning.",
		errorFetchingData: "Ma'lumotlarni yuklab bo'lmadi. Iltimos, qayta urinib ko'ring.",
		// Language settings translations
		unsaved_changes_title: "Saqlanmagan o'zgarishlar",
		language_change_unsaved_message: "Saqlanmagan o'zgarishlaringiz bor. Tilni o'zgartirishdan oldin ularni saqlashni xohlaysizmi?",
		save_and_continue: 'Saqlash va davom etish',
		discard_changes: "O'zgarishlarni bekor qilish",
		display_language: 'Displey tili',
		language_settings: 'Til sozlamalari',
		language_change_notice: "Bu ilovaning displey tilini o'zgartiradi",
		// CreateSkill page translations
		it_skills_manager: "Ko'nikmalar menejeri",
		search_skills: "Ko'nikmalarni qidirish...",
		add_new_skill: "Yangi ko'nikma qo'shish",
		skills: "Ko'nikmalar",
		no_skills_found: "Ko'nikmalar topilmadi",
		no_skills_available: "Mavjud ko'nikmalar yo'q",
		try_adjusting_search: "Qidiruv shartlarini sozlab ko'ring",
		start_adding_skill: "Birinchi IT ko'nikmangizni qo'shishdan boshlang",
		edit_skill: "Ko'nikmani tahrirlash",
		skill_name: "Ko'nikma nomi",
		choose_color: 'Rang tanlash',
		color_code: 'Rang kodi',
		update: 'Yangilash',
		create: 'Yaratish',
		confirm_delete: "O'chirishni tasdiqlash",
		delete_skill_confirmation: "Ushbu ko'nikmani o'chirishni xohlaysizmi? Bu amalni qaytarib bo'lmaydi.",
		failed_fetch_skills: "Ko'nikmalarni yuklashda xatolik",
		skill_name_required: "Ko'nikma nomi majburiy",
		skill_updated_success: "Ko'nikma muvaffaqiyatli yangilandi",
		skill_created_success: "Ko'nikma muvaffaqiyatli yaratildi",
		failed_save_skill: "Ko'nikmani saqlashda xatolik",
		skill_deleted_success: "Ko'nikma muvaffaqiyatli o'chirildi",
		failed_delete_skill: "Ko'nikmani o'chirishda xatolik",
		// Recruiter file translations
		company_documents: 'Materiallar',
		upload_info: '3 tagacha fayl (PDF, Word, Excel, PowerPoint) yuklashingiz mumkin. Maksimal hajm: 20MB',
		select_files: 'Fayllarni tanlash',
		selected_files: 'Tanlangan fayllar',
		uploading: 'Yuklanmoqda...',
		upload: 'Yuklash',
		max_files_info: "Maksimal fayllar soniga (3 ta) yetdingiz. Yangi fayllar yuklash uchun mavjud fayllarni o'chiring.",
		max_files_reached: 'Maksimal fayllar soniga yetildi (3 ta fayl)',
		can_upload_only: 'Faqat yuklash mumkin:',
		more_files: 'ta fayl',
		invalid_file_type: "Noto'g'ri fayl turi",
		insufficient_space: 'Joy yetarli emas. Mavjud',
		available: 'Mavjud',
		space_used: 'Ishlatilgan joy',
		file_too_large: 'Fayl juda katta',
		total_size_exceeded: 'Umumiy hajm 20MB dan oshdi',
		files_uploaded_successfully: 'Fayllar muvaffaqiyatli yuklandi',
		upload_failed: 'Yuklashda xatolik',
		no_files_uploaded: 'Hali fayllar yuklanmagan',
		no_intro_pages_set: 'Hali sahifalar belgilanmagan',
		download_failed: 'Yuklab olishda xatolik',
		delete_file: "Faylni o'chirish",
		delete_file_confirm: "Bu faylni o'chirishni xohlaysizmi?",
		deleting: "O'chirilmoqda...",
		file_deleted: "Fayl muvaffaqiyatli o'chirildi",
		delete_failed: "O'chirishda xatolik",
		// Recruiter profile (override/add ensure available in UZ)
		addNews: "Yangilik qo'shish",
		noNewsFound: 'Yangiliklar topilmadi',
		noNewsAvailable: 'Yangiliklar mavjud emas',
		tryAdjusting: "Qidiruv shartlarini sozlab ko'ring",
		noImageAvailable: 'Rasm mavjud emas',
		confirmDeleteNews: "Bu yangilikni o'chirishni xohlaysizmi?",
		hashtagPlaceholder: 'masalan: texnologiya, yangilik, yangilanish',
		updateNews: 'Yangilash',
		filter: 'Filtr',
		filter_required: 'Majburiy',
		filter_optional: 'Ixtiyoriy',
		clear: 'Tozalash',
		noSkills: "Hech qanday ko'nikma mavjud emas.",
		pleaseAnswerRequired: 'Majburiy savollarga javob bering',
		// News page UI/UX additions
		readMore: "Batafsil o'qish",
		close: 'Yopish',
		retry: 'Qayta urinish',
		source: 'Manba',
		sortBy: 'Saralash',
		newest: 'Yangi avval',
		oldest: 'Eski avval',
		searchNewsPlaceholder: 'Yangiliklarni qidiring...',
		loadMore: "Ko'proq ko'rsatish",
		newsDetails: 'Yangilik tafsilotlari',
		confirmMarkAllRead: "Barcha yangiliklarni o'qilgan deb belgilaysizmi?",
		allNewsMarkedRead: "Barcha yangiliklar o'qilgan deb belgilandi.",
		login_failed: "Kirishda xatolik yuz berdi. Iltimos, qayta urinib ko'ring.",
		no_data_found: "Hech qanday ma'lumot topilmadi",
		next: 'Keyingi',
		expectedGraduationYearMonth: 'Kutilayotgan bitirish yili/oyi',
		number_of_applications: 'Arizalar soni',
		changed_elements: 'Oʻzgartirilgan elementlar',
		education: "Ta'lim",
		add_arubaito: "Arubaitoni qo'shish",
		arubaito: 'Arubaito',
		no_arubaito: 'Arubaito mavjud emas',
		licenses: 'Litsenziyalar',
		no_licenses: 'Litsenziyalar mavjud emas',
		add_license: "Litsenziya qo'shish",
		address_furigana: 'Manzil (Furigana)',
		additional_info: 'Qoʻshimcha maʼlumot',
		additional_address: 'Qoʻshimcha manzil',
		additional_address_furigana: 'Qoʻshimcha manzil (Furigana)',
		additional_email: 'Qoʻshimcha email',
		additional_phone: 'Qoʻshimcha telefon',
		additional_index: 'Qoʻshimcha indeks',
		isMarried: 'Oilaviy holat',
		married: 'Uylangan',
		single: 'Yolgʻiz',
		postal_code: 'Pochta indeksi',
		no_education: "Ta'lim yozuvlari hali yoʻq.",
		no_work_experience: 'Hali ish tajribangiz yoʻq.',
		edit_education: "Ta'limni tahrirlash",
		add_education: "Ta'lim qoʻshish",
		edit_work_experience: 'Ish tajribasini tahrirlash',
		add_work_experience: 'Ish tajribasi qoʻshish',
		present: 'Hozir',
		company: 'Kompaniya',
		role_position: 'Lavozim',
		description_label: 'Tavsif',
		from_yyyy_mm: 'Boshlanish (YYYY-MM)',
		to_yyyy_mm: 'Tugash (YYYY-MM)',
		work_experience_from_placeholder: '2023-01',
		work_experience_to_placeholder: '2024-01 yoki boʻsh qoldiring',
		institution: 'Taʼlim muassasasi',
		status: 'Holat',
		year: 'Yil',
		month: 'Oy',
		edit_arubaito: 'Qisman ishni tahrirlash',
		add_arubaito_form: 'Qisman ish qoʻshish',
		edit_license: 'Litsenziya/Sertifikatni tahrirlash',
		add_license_form: 'Litsenziya/Sertifikat qoʻshish',
		certificate_name: 'Litsenziya/Sertifikat nomi',
		loading: 'Yuklanmoqda...',
		institution_placeholder: 'masalan: Tokyo University',
		status_placeholder: 'masalan: Bitirgan, Oʻqiyapti',
		year_placeholder: '2023',
		month_placeholder: '03 (ixtiyoriy)',
		certificate_placeholder: 'masalan: TOEIC 800',
		company_placeholder: 'masalan: konbini',
		role_placeholder: 'masalan: kassir',
		period_placeholder: 'masalan: 2023-01 ~ 2024-01',
		period: 'Davr',
		download_cv: 'CV yuklab olish',
		error_loading_data: "Ma'lumotlarni yuklashda xatolik",
		error_fetching_draft: "Qoralama ma'lumotlarini olishda xatolik",
		unable_to_determine_student_id: 'Talaba ID aniqlanmadi',
		loading_student_profile: 'Profil yuklanmoqda...',
		loading_questions: 'Savollar yuklanmoqda...',
		changes_cancelled: "O'zgarishlar bekor qilindi",
		item_deleted_success: "O'chirildi!",
		error_deleting_item: "O'chirishda xatolik. Qayta urinib ko'ring.",
		error_label: 'Xatolik',
		no_valid_student_id: "To'g'ri talaba ID topilmadi",
		invalid_video_urls_removed: "Ba'zi noto'g'ri video URL'lar o'chirildi.",
		max_three_videos: "Maksimal 3 ta video qo'shish mumkin.",
		upload_failed_file_invalid: 'Fayl yuklanmadi. Fayl hajmi va formatini tekshiring.',
		required: 'Majburiy',
		add_role: "Rol qo'shish",
		url_placeholder: 'https://example.com',
		url_placeholder_page: 'https://example.com/page',
		credit_details_disabled: "Kredit tafsilotlari sahifasi hozircha o'chirilgan",
		credit_details_load_error: 'Kredit tafsilotlari yuklanmadi',
		credit_details_sync_success: 'Kredit tafsilotlari muvaffaqiyatli sinxronlashtirildi',
		credit_details_sync_failed: 'Kredit tafsilotlarini sinxronlashtirishda xatolik',
		file_size_max_5mb: 'Fayl juda katta. Maksimal hajmi 5MB.',
		invalid_file_type_images_only: "Noto'g'ri fayl turi. Faqat rasm fayllarini tanlang.",
		video_thumbnail: 'Video miniaturasi',
		company_intro_video: 'Kompaniya tanishtiruv videosi',
		video_thumbnails: 'Video miniaturalari',
		close: 'Yopish',
		close_filter_modal: 'Filtr modalini yopish',
		more: "Ko'proq",
		delete_aria: "O'chirish",
		download_aria: 'Yuklab olish',
		enrollment: "O'qishga kirish",
		graduation: 'Bitirish',
		jdu: 'JDU',
		// Mail Service
		ms_title: 'Pochta xizmati',
		ms_periodic_email: 'Davriy xat',
		ms_inactive_student_email: 'Nofaol talabalar xati',
		ms_periodic_email_settings: 'Davriy xat sozlamalari',
		ms_send_period: 'Yuborish davri',
		ms_search_period: 'Qidiruv davri',
		ms_email_subject: 'Xat mavzusi',
		ms_email_body: 'Xat matni',
		ms_plain_text_hint: "Oddiy matn sifatida yozing. Qator ko'chirishlar saqlanadi.",
		ms_inactive_description: "Qayta topshirish uchun qaytarilgan va tanlangan hafta oralig'ida qayta topshirmagan talabalarni qidiring. Har bir oraliq alohida (masalan, 1 hafta = faqat 7–13 kun oldin).",
		ms_inactive_period_title: "Qaytarilgan talabalar (hafta bo'yicha)",
		ms_1_week: '1 hafta',
		ms_2_weeks: '2 hafta',
		ms_3_weeks: '3 hafta',
		ms_4_weeks: '4 hafta',
		ms_1_month: '1 oy',
		ms_3_months: '3 oy',
		ms_6_months: '6 oy',
		ms_9_months: '9 oy',
		ms_1_year: '1 yil',
		ms_1_year_plus: "1 yil va undan ko'p",
		ms_6_months_plus: "6 oy va undan ko'p",
		ms_save: 'Saqlash',
		ms_search: 'Qidirish',
		ms_reset: 'Tozalash',
		ms_send_email: 'Xat yuborish',
		ms_students_found: 'Topilgan talabalar',
		ms_no_students_found: 'Bu davr uchun talaba topilmadi',
		ms_student_id: 'Talaba ID',
		ms_student_name: 'Ism',
		ms_email: 'Email',
		ms_last_updated: 'Oxirgi yangilanish',
		ms_last_updated_by: 'Oxirgi yangilagan',
		ms_last_activity: 'Oxirgi faollik',
		ms_never_active_title: "Hech qachon faol bo'lmagan talabalar",
		ms_never_active_description: "Ro'yxatdan o'tganidan beri bitta ham draft topshirmagan talabalarni qidiring.",
		ms_registered_at: "Ro'yxatdan o'tgan sana",
		ms_never_active_students: "Hech qachon faol bo'lmagan talabalar",
		ms_never_active_hint: "(ro'yxatdan o'tganidan beri bitta ham draft topshirmagan)",
		ms_setting_updated: 'Sozlama muvaffaqiyatli yangilandi',
		ms_error_loading: 'Sozlamalarni yuklashda xatolik',
		ms_error_updating: 'Sozlamani yangilashda xatolik',
		ms_error_searching: 'Talabalarni qidirishda xatolik',
		ms_error_sending: 'Xat yuborishda xatolik',
		ms_fill_all_fields: "Barcha maydonlarni to'ldiring",
		ms_emails_sent: 'Xatlar yuborildi',
		ms_send_result: 'Yuborish natijasi',
		ms_successful: 'muvaffaqiyatli',
		ms_failed: 'muvaffaqiyatsiz',
		ms_year: 'yil',
		ms_month: 'oy',
		ms_week: 'hafta',
		ms_day: 'kun',
		ms_days: 'kun',
		ms_schedule_day_of_month: 'Yuborish kuni (oylik)',
		ms_schedule_hour: 'Yuborish soati',
		ms_schedule_day_of_month_hint: 'Har oydagi yuborish kuni (1–28)',
		ms_schedule_hour_hint: 'Yuborish soati (0–23)',
		// Sidebar & profile header
		maintenance: 'Texnik xizmat',
		expected_graduation_month: 'Kutilayotgan bitirish sanasi',
		enrolled_partner_university: 'Hamkor universitet',
		not_set: 'Belgilanmagan',
		getLink: 'Link olish',
		generating: 'Yaratilmoqda...',
		regenerateLink: 'Yangi link',
		linkExpiresIn: 'Muddati tugashiga',
		link_expired: 'Link muddati tugagan',
		link_expiry_notice: 'Link muddati',
		link_generated_success: 'Link muvaffaqiyatli yaratildi!',
		link_generation_failed: 'Link yaratishda xatolik',
		link_copied: 'Link nusxalandi!',
		linkCopied: 'Link nusxalandi!',
		profile_not_public: "Profil tasdiqlanmagan, javola yaratib bo'maydi",
		deactivate_link: "Linkni o'chirish",
		deactivating: "O'chirilmoqda...",
		link_deactivated_success: "Link o'chirildi",
		link_deactivate_failed: "Linkni o'chirishda xatolik",
		confirm_to_download_cv: "O'qib chiqdim",
		resumeDownloadWarning: `
⚠️ Rezyumeni yuklab olishdan avval o'qing!
Hurmatli talaba,
Ushbu rezyume (CV) veb-saytdagi portfoliongiz asosida tizim tomonidan avtomatik shakllantirilgan bo'lib, uning ichidagi ma'lumotlar 100% aniq yoki yakuniy bo'lmasligi mumkin. Yuklab olganingizdan so'ng, uni "見本 履歴書" (Namuna) bilan solishtirib, qo'lda tekshirib chiqishingiz qat'iyan talab qilinadi!

❗️ Ko'pchilik xato qiladigan va alohida e'tibor berishingiz kerak bo'lgan qismlar:
1) 「職務経歴書」 (Ish tajribasi):
Ushbu qismga faqat universitetning "Cowork" tizimida ishlagan loyihalaringiz kiritilishi kerak. Ma'lumotlar avtomatik yuklanayotgani sababli aralashib qolish ehtimoli bor. Iltimos, "Cowork" va tashqaridagi loyihalarni (arubaito, frilanserlik, boshqa kompaniya loyihalari) alohida ajratib, har birini o'zining tegishli joyiga yozganingizni tekshirib oling.

2) Manzillar – 「連絡先」 va 「現住所」:
「現住所」: Bu sizning hozirgi vaqtda yashab turgan manzilingiz kiritiladigan joy.
「連絡先」: Bu esa vasiylaringiz (ota-onangiz) istiqomat qilayotgan manzil.
Agar ikkala manzil ham bir xil bo'lsa, pastki qismiga (連絡先'ga) shunchaki 「同上」 deb yozishingiz kerak. Iltimos, to'g'ri to'ldirilganligini tekshirib oling.

3) 「自己PR」 (O'zini tanishtirish / Yutuqlar):
Ushbu qismni tayyor andoza bilan qoldirib bo'lmaydi. Talaba har doim bu qismni qayerga (qaysi kompaniyaga yoki vaziyatga) topshirayotganiga qarab, moslab qaytadan yozishi talab etiladi. O'zingizga moslab tahrirlanganini tekshirib oling.
💡 Agar rezyumeni tahrirlashda tushunmovchiliklar yoki qandaydir muammolar paydo bo'lsa, iltimos, senpaylar yoki universitet xodimlariga (staff) murojaat qiling.
`,
		companies: 'Kompaniyalar',
		add_recruiter: 'Rekruter qoʻshish',
		choose_existing_company: 'Mavjud kompaniyani tanlang',
		create_new_company: 'Yangi kompaniya yaratish',
		confirm_delete_recruiter: "Bu rekruterni o'chirishni xohlaysizmi?",
		not_assign_company: "Siz kompaniyaga a'zo qilinmagansiz.",
		companies: 'Kompaniyalar',
		new_company: 'Yangi kompaniya',
		company_name: 'Kompaniya nomi',
		company_name_placeholder: "Nomi, manzili, vakili bo'yicha qidirish...",
		is_partner_true: 'Faqat hamkor',
		is_partner_false: 'Faqat oddiy',
		is_partner: 'Hamkorlik',
		partner_chip: 'Hamkor',
		set_parent_recruiter: 'Asosiy rekruter etib tayinlandi',
		removed_parent_recruiter: 'Asosiy rekruterlikdan olib tashlandi',
		assign_recruiter: 'Rekruter biriktirish',
		company_address: 'Kompaniya manzili',
		recruiters: 'Rekruterlar',
		no_companies_found: "Topilgan kompaniyalar yo'q",
		create_company: 'Yangi kompaniya yaratish',
		company_identity: 'Kompaniya identifikatori',
		no_recruiters_found: "Topilgan rekruterlar yo'q",
		search_recruiters: 'Rekruterlarni qidirish...',
		select_company: 'Kompaniyani tanlang',
		assigned_recruiter_to_company: 'Rekruter kompaniyaga biriktirildi',
		unassigned_recruiter_from_company: 'Rekruter kompaniyadan olib tashlandi',
		updated_company_identity: 'Kompaniya identifikatori yangilandi',
		updated_company_profile: 'Kompaniya profili yangilandi',
		details: 'Tafsilotlar',
		edit_recruiter: 'Rekruterni tahrirlash',
		company_change_hint: 'Kompaniyani faqat kompaniya sahifasidan o‘zgartirish mumkin.',
	},
	ru: {
		age: 'Возраст',
		rank: 'Ранг',
		title: 'Заголовок',
		description: 'Описание',
		sourseLink: 'Ссылка',
		hashtag: 'Хэштег',
		CreateNews: 'Создать новость',
		EditNews: 'Редактировать новость',
		visibleToRecruiter: 'Показывать рекрутеру',
		newsVisibility: 'Видимость новостей',
		universityNews: 'Университетские новости',
		universityNewsDescription: 'Видны всем (сотрудники, администраторы, студенты, рекрутеры)',
		recruiterNews: 'Новости для рекрутеров',
		recruiterNewsDescription: 'Видны всем, кроме рекрутеров',
		imageRequired: 'Изображение обязательно',
		descriptionRequired: 'Описание обязательно',
		fillRequiredFields: 'Пожалуйста, заполните все обязательные поля (Заголовок, Описание и Изображение)',
		cencel: 'Отмена',
		searchByHashtag: 'Поиск по хэштегу, названию компании или заголовку новости...',
		checkOut: 'Ознакомьтесь с последними объявлениями и предложениями от компаний',
		newsHighlights: 'Основные новости',
		news: 'Новости компании',
		Qualifications: 'Квалификации',
		studetentSkillManagment: 'Управление навыками студентов',
		studentCredits: 'Количество кредитов студента',
		origin: 'Происхождение',
		major: 'Специальность',
		jobType: 'Желаемая работа',
		qualification: 'Квалификация',
		japaneseSpeechContest: 'Конкурс речей на японском языке',
		itContest: 'IT-конкурс',
		languageSkills: 'Языковые навыки',
		japanese: 'Японский',
		general: 'Общий',
		home: 'Главная',
		profile: 'Профиль',
		student_search: 'Поиск студентов',
		student_check: 'Проверка студентов',
		staff: 'Персонал',
		recruiter: 'Рекрутер',
		bookmarked: 'Закладки',
		settings: 'Настройки',
		help: 'Помощь',
		student_qa: 'Вопросы студентов',
		logout: 'Выйти',
		Are_you_sure: 'Вы уверены, что хотите выйти?',
		yesModal: 'Да',
		noModal: 'Нет',
		jdu_portfolio: 'JDU Портфолио',
		japan: 'Япония',
		uzbekistan: 'Узбекистан',
		next_button: 'Перейти к поиску студентов ➜',
		large_class_photo_alt: 'Большое фото класса',
		group_photo_alt: 'Групповое фото',
		changes_saved: 'Изменения успешно сохранены!',
		grade: 'Курс',
		grade1: '1 курс',
		grade2: '2 курс',
		grade3: '3 курс',
		grade4: '4 курс',
		programming_languages: 'Языки программирования',
		any: 'Любой',
		all: 'Все',
		search_items: 'Поиск...',
		select_all_filtered: 'Выбрать все (фильтр)',
		clear_filtered: 'Очистить (фильтр)',
		selected: 'Выбрано',
		select_all: 'Выбрать все',
		clear_all: 'Снять все',
		jlpt: 'Тест на знание японского языка (JLPT)',
		ielts: 'IELTS',
		jdu_certification: 'Тест JDU по японскому языку',
		credits: 'Кредиты',
		credits_unit: ' Кредиты в рамках',
		partner_university: 'Университет-партнёр',
		special_skills: 'Особые навыки',
		yes: 'Да',
		no: 'Нет',
		tokyo_communication_university: 'Токийский университет коммуникаций',
		kyoto_tachibana_university: 'Университет Киото Тачибана',
		sanno_university: 'Университет Санно',
		sanno_junior_college: 'Краткосрочный колледж Санно',
		niigata_sangyo_university: 'Ниигатский промышленный университет',
		otemae_university: 'Университет Отэмаэ',
		okayama_university_of_science: 'Университет науки Окаямы',
		unaffiliated: 'Не прикреплён',
		student: 'Студент',
		rows_per_page: 'Строк на странице:',
		search: 'Поиск',
		reset: 'Сброс',
		name_search: 'Поиск по имени',
		department: 'Факультет',
		position: 'Должность',
		phone_number: 'Номер телефона',
		name: 'Имя',
		company_name: 'Компания',
		edit: 'Редактировать',
		cancel: 'Отмена',
		save: 'Сохранить',
		sync: 'Синхронизировать',
		user_avatar: 'Аватар пользователя',
		company_logo: 'Логотип компании',
		upload_picture: 'Загрузить изображение',
		user: 'Пользователь',
		first_name: 'Имя',
		first_name_furigana: 'Имя (фуригана)',
		last_name: 'Фамилия',
		last_name_furigana: 'Фамилия (фуригана)',
		furigana_help: 'Введите произношение хираганой/катаканой',
		phone: 'Телефон',
		email: 'Электронная почта',
		change_password: 'Сменить пароль',
		current_password: 'Пароль',
		new_password: 'Новый пароль',
		confirm_password: 'Подтвердите пароль',
		toggle_password_visibility: 'Показать/скрыть пароль',
		contact_info: 'Контактная информация',
		contact_email: 'Контактная почта',
		contact_phone: 'Контактный телефон',
		working_hours: 'Рабочее время',
		location: 'Местоположение',
		user_fetch_error: 'Не удалось получить данные пользователя.',
		profile_update_success: 'Профиль успешно обновлен.',
		profile_update_failed_retry: 'Ошибка при обновлении профиля. Попробуйте снова.',
		unknown_role_error: 'Неизвестная роль.',
		password_mismatch: 'Пароли не совпадают.',
		current_password_required: 'Введите текущий пароль.',
		unknown_role: 'Неизвестная роль.',
		fetch_user_failed: 'Не удалось получить данные пользователя.',
		profile_updated: 'Профиль успешно обновлен.',
		profile_update_failed: 'Ошибка при обновлении профиля. Попробуйте снова.',
		sync_successful: 'Синхронизация прошла успешно.',
		sync_failed: 'Ошибка синхронизации. Попробуйте снова.',
		notFoundTitle: '404 - Страница не найдена',
		notFoundMessage: 'Страница, которую вы ищете, не существует.',
		backToHome: 'На главную',
		maintenanceTitle: 'На техническом обслуживании',
		checkingMaintenance: 'Проверка статуса обслуживания...',
		checkingStatus: 'Автоматическая проверка статуса...',
		// Maintenance page translations
		maintenance_announcement: 'Объявление о техническом обслуживании',
		announcement_settings: 'Настройки объявления',
		message: 'Сообщение',
		enter_announcement_message: 'Введите текст объявления...',
		visibility_settings: 'Настройки видимости',
		make_public: 'Сделать объявление видимым для всех пользователей на странице входа',
		announcement_visible_to_all_users: 'Это объявление в настоящее время видно всем пользователям на странице входа.',
		announcement_hidden_from_users: 'Это объявление в настоящее время скрыто от пользователей.',
		created_at: 'Создано',
		last_updated: 'Последнее обновление',
		message_required: 'Сообщение обязательно',
		announcement_updated: 'Объявление успешно обновлено',
		error_updating_announcement: 'Ошибка при обновлении объявления',
		please_update_announcement_first: 'Пожалуйста, сначала обновите объявление',
		announcement_made_public: 'Объявление теперь видно всем пользователям',
		announcement_made_private: 'Объявление теперь скрыто от пользователей',
		error_toggling_status: 'Ошибка при переключении статуса',
		unauthorizedTitle: '403 - Доступ запрещен',
		unauthorizedMessage: 'У вас нет доступа к этой странице.',
		unauthorizedButton: 'Перейти на страницу входа',
		back: 'Назад',
		add: 'Добавить',
		saveDraft: 'Новый черновик',
		updateDraft: 'Обновить черновик',
		submitAgree: 'Отправить/Согласие',
		edit_profile: 'Редактировать профиль',
		liveProfile: 'Живой профиль',
		draftProfile: 'Черновик профиля',
		qa_edit: 'Редактировать Q&A',
		faq_heading: 'Частые вопросы',
		faq_no_answer: 'Ответа пока нет',
		q_edit: 'Редактировать вопросы',
		company_overview: 'Обзор компании',
		age: 'Возраст',
		student_id: 'ID студента',
		top: 'Верх',
		qa: 'Q&A',
		stats: 'Кредиты и навыки',
		errorFetchingStudent: 'Ошибка при получении данных студента.',
		changesSavedSuccessfully: 'Изменения успешно сохранены!',
		errorSavingChanges: 'Ошибка при сохранении изменений.',
		errorUploadingFile: 'Ошибка при загрузке файла.',
		savingChanges: 'Сохранение изменений...',
		changesSaved: 'Изменения сохранены',
		dataRecovered: 'Данные успешно восстановлены',
		recoverUnsavedChanges: 'Восстановить несохраненные изменения?',
		unsavedChangesFound: 'Мы нашли несохраненные изменения из вашей предыдущей сессии редактирования. Восстановить их?',
		lastModified: 'Последнее изменение',
		discard: 'Отменить',
		restore: 'Восстановить',
		unsavedChangesTitle: 'Несохраненные изменения',
		unsavedChangesMessage: 'У вас есть несохраненные изменения. Вы уверены, что хотите их отменить?',
		continueEditing: 'Продолжить редактирование',
		discardChanges: 'Отменить изменения',
		unsavedChangesLanguageTitle: 'Сохранить изменения перед сменой языка?',
		unsavedChangesLanguageMessage: 'У вас есть несохраненные изменения. Хотите сохранить их перед сменой языка?',
		discardAndSwitch: 'Отменить и переключить',
		saveAndSwitch: 'Сохранить и переключить',
		dataRestoredAfterLanguageSwitch: 'Ваши данные восстановлены после смены языка',
		unsavedChangesNavigationTitle: 'Сохранить изменения перед уходом?',
		unsavedChangesNavigationMessage: 'У вас есть несохраненные изменения. Хотите сохранить их перед уходом со страницы?',
		discardAndLeave: 'Отменить и выйти',
		saveAndLeave: 'Сохранить и выйти',
		fileUploadSuccess: 'Файл успешно загружен.',
		selfIntroduction: 'Самопрезентация',
		deliverables: 'Результаты',
		hobbies: 'Хобби',
		specialSkills: 'Особые навыки',
		itSkills: 'IT-навыки',
		otherSkills: 'Другие навыки',
		advanced: 'Продвинутый',
		intermediate: 'Средний',
		beginner: 'Начинающий',
		threeYearsOrMore: 'Три года и более',
		oneToOneAndHalfYears: 'От одного до полутора лет',
		lessThanThreeYears: 'Менее трёх лет',
		basic: 'Базовый',
		editProfile: 'Редактировать профиль',
		skillExists: 'Навык уже существует на этом уровне!',
		skillAdded: 'Навык успешно добавлен!',
		skillDeleted: 'Навык успешно удален!',
		selectSkill: 'Выберите навык',
		skill: 'Навык',
		level: 'Уровень',
		languageName: 'Название языка',
		awardName: 'Название награды/сертификата',
		noLanguageSkillsEdit: 'Навыки пока не добавлены',
		noSkillsEdit: 'Навыки ещё не добавлены. Используйте форму выше, чтобы добавить навыки.',
		noLanguageSkills: 'Нет доступных навыков',
		noAwardsEdit: 'Награды пока не добавлены',
		noAwards: 'Нет доступных наград',
		levelInfo: 'Продвинутый: более 3 лет | Средний: менее 3 лет | Начинающий: менее 1 года',
		notEntered: 'Не заполнено',
		not_submitted: 'Не представлено',
		changed: 'Изменено',
		none: 'Нет',
		// ChangedFieldsModal
		changed_elements_list: 'Список изменённых полей',
		changes_count_message: 'Всего изменений: %d',
		changes_count_short: '%d изменений',
		no_changes: 'Нет изменений',
		category_basic_info: 'Основная информация',
		category_skills_abilities: 'Навыки и умения',
		category_interests: 'Увлечения',
		category_certifications: 'Квалификации',
		category_portfolio: 'Портфолио',
		category_qa: 'Вопросы и ответы',
		category_other: 'Прочее',
		gallery: 'Галерея',
		hobbies_description: 'Описание увлечений',
		special_skills_description: 'Описание навыков',
		showMore: 'Показать ещё',
		showLess: 'Скрыть',
		addDeliverable: 'Добавить результат',
		createDeliverable: 'Создать результат',
		editDeliverable: 'Редактировать результат',
		noDeliverables: 'Результатов пока нет',
		addFirstDeliverable: 'Добавьте ваш первый результат',
		deliverableCreated: 'Результат успешно создан!',
		deliverableUpdated: 'Результат успешно обновлен!',
		deliverableDeleted: 'Результат успешно удален!',
		titleRequired: 'Заголовок обязателен',
		filesRequired: 'Необходимо как минимум одно изображение',
		selectImages: 'Выберите изображения',
		addMoreImages: 'Добавить ещё изображения',
		replaceImages: 'Заменить изображения',
		currentImages: 'Текущие изображения',
		newImages: 'Новые изображения',
		images: 'Изображения',
		view: 'Просмотр',
		openLink: 'Открыть ссылку',
		openCodeLink: 'Открыть ссылку на код',
		allImages: 'Все изображения',
		confirmDelete: 'Вы уверены, что хотите удалить этот результат?',
		levels: {
			上級: 'Продвинутый: более 3 лет',
			中級: 'Средний: менее 3 лет',
			初級: 'Начинающий: менее 1 года',
			JLPT: 'JLPT',
			JDU_JLPT: 'Тест JDU по японскому языку',
			IELTS: 'IELTS',
			日本語弁論大会学内: 'Конкурс речей на японском языке',
			ITコンテスト学内: 'IT-конкурс',
		},
		approved: 'Одобрено',
		disapproved: 'Отклонено',
		resubmission_required: 'Требуется повторная отправка',
		submitted: 'Отправлено',
		draft: 'Черновик',
		published: 'Опубликовано',
		private: 'Приватный',
		notifications: 'Уведомления',
		mark_all_read: 'Отметить все как прочитанные',
		mark_as_read: 'Отметить как прочитанное',
		readed: 'Прочитано',
		unread: 'Непрочитанные',
		read: 'Прочитанные',
		expand: 'Развернуть',
		collapse: 'Свернуть',
		no_notifications: 'Пока нет уведомлений...',
		no_unread_notifications: 'Нет непрочитанных уведомлений',
		no_read_notifications: 'Нет прочитанных уведомлений',
		new: 'НОВОЕ',
		ok: 'ОК',
		delete: 'Удалить',
		loading: 'Загрузка...',
		warning: 'Предупреждение',
		confirmApprove: 'Одобрить?',
		confirmSendBack: 'Отправить обратно?',
		confirmApproveDesc: 'Это действие отправит уведомление об «одобрении» студенту. Хотите продолжить?',
		confirmSendBackDesc: 'Это действие отправит уведомление об отклонении с комментариями студенту. Хотите продолжить?',
		staffFeedbackTitle: 'Комментарий от сотрудника',
		staffFeedbackHint: 'Пожалуйста, исправьте профиль с учётом этого комментария и отправьте снова.',
		pastStaffComments: 'Предыдущие комментарии сотрудников',
		staffComment: 'Комментарий',
		resubmissionComment: 'Комментарий к возврату',
		approve: 'Одобрить',
		disapprove: 'Не одобрять',
		sendBack: 'Отправить обратно',
		originPlaceholder: 'Uzbekistan',
		addressFuriganaPlaceholder: 'ウズファイスト',
		studentNotApprovedByStaff: 'Студент должен быть одобрен персоналом',
		profileVisibilityEnabled: 'Видимость профиля успешно включена',
		profileHidden: 'Профиль успешно скрыт',
		errorSettingVisibility: 'Ошибка при установке видимости',
		navigationWarningTitle: 'Несохраненные изменения',
		navigationWarningMessage: 'У вас есть несохраненные изменения. Отменить их и покинуть страницу?',
		welcome: 'JDU Портфолио',
		forgotPassword: 'Забыли пароль',
		resetPassword2: 'Введите свой email, чтобы получить инструкции по сбросу пароля',
		enterYourLogin: 'Введите вашу почту',
		enterYourPassword: 'Введите ваш пароль',
		mailAddress: 'Адрес электронной почты',
		send: 'Отправить письмо',
		loginLabel: 'Войти',
		profile_publish_request: 'Заявка на публикацию',
		profile_publish_explanation: 'Мы опубликуем ваш профиль в сервисе JDU Portfolio для облегчения поиска работы. После подачи заявки она будет отправлена в Отдел трудоустройства JDU в Японии, где сотрудник проверит содержание. Если будут вопросы, мы свяжемся с вами по почте. При отсутствии вопросов ваша заявка будет одобрена, и профиль станет публичным.',
		prohibited_actions: 'Запрещенные действия',
		prohibited_actions_content: 'Студентам JDU запрещено: <ol><li>Предоставлять ложную или неточную информацию</li><li>Оскорблять, клеветать, угрожать людям или организациям</li><li>Нарушать права других (авторские, товарные знаки и т.д.)</li><li>Мешать работе сервиса или наносить вред репутации</li><li>Совершать преступления или нарушать закон</li><li>Сотрудничать с антисоциальными организациями</li></ol>',
		confirm_no_prohibited_actions: 'Я подтверждаю, что вышеуказанные запрещённые действия ко мне не относятся.',
		no_button: 'Нет',
		apply_button: 'Отправить',
		draftSubmittedSuccessfully: 'Черновик успешно отправлен!',
		noDraftToSubmit: 'Нет черновика для отправки',
		errorSubmittingDraft: 'Ошибка при отправке черновика',
		noDataFound: 'Данные не найдены',
		start_checking: 'Начать проверку',
		addHobby: 'Добавить хобби',
		addSpecialSkill: 'Добавить особый навык',
		addDescription: 'Добавить описание',
		addTag: 'Добавить тег',
		hobbiesDetailDescription: 'Подробное описание хобби',
		specialSkillsDetailDescription: 'Подробное описание особых навыков',
		hobbiesTagPlaceholder: 'Введите теги хобби',
		specialSkillsTagPlaceholder: 'Введите теги особых навыков',
		hobbiesDescriptionPlaceholder: 'Опишите свои хобби подробно',
		specialSkillsDescriptionPlaceholder: 'Опишите свои особые навыки подробно',
		hobbiesTags: 'Теги хобби',
		specialSkillsTags: 'Теги особых навыков',
		statusUpdatedSuccessfully: 'Статус успешно обновлен',
		errorUpdatingStatus: 'Ошибка при обновлении статуса',
		checking: 'Проверка',
		not_approved_yet: 'Еще не одобрено',
		approvalStatus: 'Статус одобрения',
		approval_status_unconfirmed: 'Не подтверждено',
		approval_status_in_review: 'На проверке',
		approval_status_returned: 'Возвращено',
		approval_status_approved: 'Одобрено',
		visible_status: 'Статус видимости',
		visible: 'Виден',
		invisible: 'Скрыт',
		personal_info: 'Личная информация',
		search_placeholder: 'Поиск...',
		search_filters: 'Фильтры поиска',
		no_matches_found: 'Совпадений не найдено',
		start_typing: 'Начните ввод, чтобы увидеть предложения',
		entrance: 'Поступление',
		graduation: 'Выпуск',
		credits_unit_short: 'Кредиты',
		qualifications: 'Квалификации',
		student_grades: 'Оценки студента',
		specialized_knowledge: 'Специализированные знания',
		personality: 'Личность',
		work_experience: 'Опыт работы',
		career_goals: 'Карьера/Цели',
		comments: 'Комментарии',
		company_introduction_video: 'Видео-презентация компании',
		student_label: 'Студент',
		age_label: 'Возраст',
		age_unit: ' лет',
		application_date: 'Дата заявки',
		application_count: 'Количество заявок',
		confirmation_status: 'Статус подтверждения',
		unconfirmed: 'Не подтверждено',
		confirmed: 'Подтверждено',
		company_video_url_placeholder: 'например: https://youtu.be/rSRpRd1E45w?si=3r7PqVgWt67ZA2i5',
		save_button: 'Сохранить',
		no_video_set: 'Видео не установлено',
		company_introduction_video_description: 'Вы можете посмотреть видео-презентацию компании, предоставленное работодателем',
		new_target_audience_add: 'Добавить новую целевую аудиторию',
		required_skills_placeholder: 'например: Опыт работы с HTML/CSS/JavaScript (3+ года)',
		preferred_skills_placeholder: 'например: Опыт работы с TypeScript',
		business_overview: 'Обзор бизнеса',
		business_content_placeholder: 'Введите описание деятельности',
		recruitment_requirements: 'Требования к вакансии',
		required_skills: 'Необходимые навыки',
		welcome_skills: 'Желательные навыки',
		target_audience: 'Целевая аудитория',
		target_person: 'Целевой кандидат',
		target_audience_placeholder: 'Введите целевую аудиторию',
		additional_info: 'Дополнительная информация',
		work_location: 'Место работы',
		// Страница-введение
		intro_page: 'Страница-введение',
		intro_page_thumbnail: 'Изображение для страницы-введения',
		intro_page_links: 'Ссылки на странице-введения',
		newIntroLinkTitle: 'Название ссылки',
		newIntroLinkUrl: 'URL ссылки',
		intro_link_title: 'Название ссылки',
		max_four_links: 'Можно добавить до 4 ссылок',
		// Кадровые опции
		retirement_benefit: 'Выходное пособие',
		telework_availability: 'Возможность удаленной работы',
		housing_availability: 'Наличие общежития/служебного жилья',
		relocation_support: 'Оплата авиабилетов/расходов на переезд',
		airport_pickup: 'Встреча в аэропорту',
		work_location_placeholder: 'Введите место работы',
		work_hours: 'Рабочие часы',
		work_hours_placeholder: 'Введите рабочее время',
		salary: 'Зарплата',
		salary_placeholder: 'Введите зарплату',
		benefits: 'Льготы',
		benefits_placeholder: 'Введите льготы',
		allowances_placeholder: 'Введите другие надбавки и льготы',
		selection_process: 'Процесс отбора',
		selection_process_placeholder: 'Введите процесс отбора',
		established: 'Основана',
		established_placeholder: 'Введите дату основания',
		employee_count: 'Количество сотрудников',
		employee_count_placeholder: 'Введите количество сотрудников',
		location_placeholder: 'Введите местоположение',
		company_description_placeholder: 'Введите описание компании',
		saving: 'Сохранение...',
		unsaved_changes_title: 'Несохраненные изменения',
		language_change_unsaved_message: 'У вас есть несохраненные изменения. Хотите сохранить их перед сменой языка?',
		save_and_continue: 'Сохранить и продолжить',
		discard_changes: 'Отменить изменения',
		display_language: 'Язык отображения',
		language_settings: 'Настройки языка',
		language_change_notice: 'Это изменит язык отображения приложения',
		it_skills_manager: 'Менеджер IT-навыков',
		search_skills: 'Поиск навыков...',
		add_new_skill: 'Добавить новый навык',
		skills: 'Навыки',
		no_skills_found: 'Навыки не найдены',
		no_skills_available: 'Нет доступных навыков',
		try_adjusting_search: 'Попробуйте изменить условия поиска',
		start_adding_skill: 'Начните с добавления первого IT-навыка',
		edit_skill: 'Редактировать навык',
		skill_name: 'Название навыка',
		choose_color: 'Выберите цвет',
		color_code: 'Код цвета',
		update: 'Обновить',
		create: 'Создать',
		confirm_delete: 'Подтвердите удаление',
		delete_skill_confirmation: 'Вы уверены, что хотите удалить этот навык? Это действие необратимо.',
		failed_fetch_skills: 'Не удалось загрузить навыки',
		skill_name_required: 'Название навыка обязательно',
		skill_updated_success: 'Навык успешно обновлен',
		skill_created_success: 'Навык успешно создан',
		failed_save_skill: 'Ошибка при сохранении навыка',
		skill_deleted_success: 'Навык успешно удален',
		failed_delete_skill: 'Ошибка при удалении навыка',
		company_documents: 'Документы компании',
		upload_info: 'Можно загрузить до 3 файлов (PDF, Word, Excel, PowerPoint). Максимальный общий размер: 20MB',
		select_files: 'Выбрать файлы',
		selected_files: 'Выбранные файлы',
		uploading: 'Загрузка...',
		upload: 'Загрузить',
		max_files_info: 'Вы достигли максимального количества файлов (3). Удалите старые, чтобы загрузить новые.',
		max_files_reached: 'Максимум файлов достигнут (3 файла)',
		can_upload_only: 'Можно загрузить только',
		more_files: 'файл(а)',
		invalid_file_type: 'Недопустимый тип файла',
		insufficient_space: 'Недостаточно места. Доступно',
		available: 'Доступно',
		space_used: 'Использовано',
		file_too_large: 'Файл слишком большой',
		total_size_exceeded: 'Общий размер превысил 20MB',
		files_uploaded_successfully: 'Файлы успешно загружены',
		upload_failed: 'Ошибка загрузки',
		no_files_uploaded: 'Файлы еще не загружены',
		no_intro_pages_set: 'Страницы еще не установлены',
		download_failed: 'Ошибка загрузки файла',
		delete_file: 'Удалить файл',
		delete_file_confirm: 'Вы уверены, что хотите удалить этот файл?',
		deleting: 'Удаление...',
		file_deleted: 'Файл успешно удален',
		delete_failed: 'Ошибка удаления',
		tagline: 'PR в одну строку',
		company_information: 'Информация о компании',
		company_Address: 'Адрес',
		company_website: 'Веб-сайт',
		company_description: 'Описание компании',
		employee_Count: 'Количество сотрудников',
		established_Date: 'Дата основания',
		company_capital: 'Уставной капитал',
		company_revenue: 'Годовой доход',
		company_representative: 'Представитель',
		job_details: 'Информация о вакансии',
		job_title: 'Название должности',
		job_description: 'Описание работы',
		japanese_level: 'Уровень японского языка',
		application_requirements_other: 'Другие требования к кандидату',
		recommended_skills: 'Рекомендуемые навыки и опыт',
		recommended_licenses: 'Рекомендуемые лицензии и сертификаты',
		recommended_other: 'Рекомендуемые прочие требования',
		number_of_openings: 'Количество вакансий',
		employment_type: 'Тип занятости',
		probation_period: 'Испытательный срок',
		employment_period: 'Срок найма',
		compensation_benefits: 'Зарплата и льготы',
		salary_increase: 'Повышение зарплаты',
		bonus: 'Бонус',
		allowances: 'Надбавки',
		holidays_vacation: 'Отпуска и выходные',
		other: 'Другое',
		other_notes: 'Прочее',
		selection_info: 'Информация о процессе отбора',
		interview_method: 'Формат собеседования',
		addNews: 'Добавить новость',
		noNewsFound: 'Новости не найдены',
		noNewsAvailable: 'Нет доступных новостей',
		tryAdjusting: 'Попробуйте изменить условия поиска',
		noImageAvailable: 'Изображение недоступно',
		confirmDeleteNews: 'Вы уверены, что хотите удалить эту новость?',
		// News page UI/UX additions
		readMore: 'Читать далее',
		close: 'Закрыть',
		retry: 'Повторить',
		source: 'Источник',
		sortBy: 'Сортировать',
		newest: 'Сначала новые',
		oldest: 'Сначала старые',
		searchNewsPlaceholder: 'Поиск новостей...',
		loadMore: 'Показать еще',
		newsDetails: 'Подробнее о новости',
		confirmMarkAllRead: 'Отметить все новости как прочитанные?',
		allNewsMarkedRead: 'Все новости помечены как прочитанные.',
		hashtagPlaceholder: 'например: технологии, новости, обновление',
		updateNews: 'Обновить',
		filter: 'Фильтр',
		filter_required: 'Обязательно',
		filter_optional: 'Необязательно',
		clear: 'Очистить',
		apply: 'Применить',
		noSkills: 'Нет доступных навыков.',
		pleaseAnswerRequired: 'Пожалуйста, ответьте на все обязательные вопросы',
		login_failed: 'Ошибка входа. Пожалуйста, проверьте свои учетные данные и попробуйте снова.',
		no_data_found: 'Ничего не найдено',
		next: 'Далее',
		expectedGraduationYearMonth: 'Ожидаемый год/месяц выпуска',
		number_of_applications: 'Количество заявок',
		changed_elements: 'Измененные элементы',
		education: 'Образование',
		add_arubaito: 'Добавить подработку',
		arubaito: 'Подработка',
		no_arubaito: 'Нет подработок',
		licenses: 'Лицензии и сертификаты',
		no_licenses: 'Нет лицензий и сертификатов',
		add_license: 'Добавить лицензию или сертификат',
		address_furigana: 'Адрес (фуригана)',
		additional_info: 'Дополнительная информация',
		additional_address: 'Дополнительный адрес',
		additional_address_furigana: 'Дополнительный адрес (фуригана)',
		additional_email: 'Дополнительный email',
		additional_phone: 'Дополнительный телефон',
		additional_index: 'Дополнительный индекс',
		isMarried: 'Семейное положение',
		married: 'Женат',
		single: 'Не женат',
		postal_code: 'Почтовый индекс',
		no_education: 'Записей об образовании пока нет.',
		no_work_experience: 'У вас пока нет опыта работы.',
		edit_education: 'Редактировать образование',
		add_education: 'Добавить образование',
		edit_work_experience: 'Редактировать опыт работы',
		add_work_experience: 'Добавить опыт работы',
		present: 'По настоящее время',
		company: 'Компания',
		role_position: 'Должность',
		description_label: 'Описание',
		from_yyyy_mm: 'Начало (ГГГГ-ММ)',
		to_yyyy_mm: 'Конец (ГГГГ-ММ)',
		work_experience_from_placeholder: '2023-01',
		work_experience_to_placeholder: '2024-01 или оставьте пустым',
		institution: 'Учебное заведение',
		status: 'Статус',
		year: 'Год',
		month: 'Месяц',
		edit_arubaito: 'Редактировать подработку',
		add_arubaito_form: 'Добавить подработку',
		edit_license: 'Редактировать лицензию/сертификат',
		add_license_form: 'Добавить лицензию/сертификат',
		certificate_name: 'Название лицензии/сертификата',
		loading: 'Загрузка...',
		institution_placeholder: 'напр. Токийский университет',
		status_placeholder: 'напр. Окончил, Учусь',
		year_placeholder: '2023',
		month_placeholder: '03 (необязательно)',
		certificate_placeholder: 'напр. TOEIC 800',
		company_placeholder: 'напр. мини-маркет',
		role_placeholder: 'напр. кассир',
		period_placeholder: 'напр. 2023-01 ~ 2024-01',
		period: 'Период',
		download_cv: 'Скачать резюме',
		error_loading_data: 'Ошибка загрузки данных',
		error_fetching_draft: 'Ошибка загрузки черновика',
		unable_to_determine_student_id: 'Не удалось определить ID студента',
		loading_student_profile: 'Загрузка профиля студента...',
		loading_questions: 'Загрузка вопросов...',
		changes_cancelled: 'Изменения отменены',
		item_deleted_success: 'Удалено!',
		error_deleting_item: 'Ошибка удаления. Попробуйте снова.',
		error_label: 'Ошибка',
		no_valid_student_id: 'Действительный ID студента не найден',
		invalid_video_urls_removed: 'Некоторые недействительные URL видео удалены.',
		max_three_videos: 'Можно добавить до 3 видео.',
		upload_failed_file_invalid: 'Ошибка загрузки файла. Проверьте размер и формат файла.',
		required: 'Обязательно',
		add_role: 'Добавить роль',
		url_placeholder: 'https://example.com',
		url_placeholder_page: 'https://example.com/page',
		credit_details_disabled: 'Страница кредитов отключена',
		credit_details_load_error: 'Не удалось загрузить данные о кредитах',
		credit_details_sync_success: 'Данные о кредитах успешно синхронизированы',
		credit_details_sync_failed: 'Ошибка синхронизации данных о кредитах',
		file_size_max_5mb: 'Файл слишком большой. Максимум 5 МБ.',
		invalid_file_type_images_only: 'Недопустимый тип файла. Выберите только изображения.',
		video_thumbnail: 'Миниатюра видео',
		company_intro_video: 'Видео о компании',
		video_thumbnails: 'Миниатюры видео',
		close: 'Закрыть',
		close_filter_modal: 'Закрыть окно фильтра',
		more: 'Ещё',
		delete_aria: 'Удалить',
		download_aria: 'Скачать',
		enrollment: 'Поступление',
		graduation: 'Выпуск',
		jdu: 'JDU',
		// Mail Service
		ms_title: 'Почтовый сервис',
		ms_periodic_email: 'Периодическая рассылка',
		ms_inactive_student_email: 'Неактивные студенты',
		ms_periodic_email_settings: 'Настройки периодической рассылки',
		ms_send_period: 'Период отправки',
		ms_search_period: 'Период поиска',
		ms_email_subject: 'Тема письма',
		ms_email_body: 'Текст письма',
		ms_plain_text_hint: 'Пишите обычным текстом. Переносы строк сохраняются.',
		ms_inactive_description: 'Поиск студентов, которым вернули профиль и которые не отправляли повторно в выбранном недельном интервале. Каждый интервал эксклюзивный (напр. 1 неделя = только 7–13 дней назад).',
		ms_inactive_period_title: 'Возвращенные студенты (по неделям)',
		ms_1_week: '1 неделя',
		ms_2_weeks: '2 недели',
		ms_3_weeks: '3 недели',
		ms_4_weeks: '4 недели',
		ms_1_month: '1 месяц',
		ms_3_months: '3 месяца',
		ms_6_months: '6 месяцев',
		ms_9_months: '9 месяцев',
		ms_1_year: '1 год',
		ms_1_year_plus: '1 год и более',
		ms_6_months_plus: '6 месяцев и более',
		ms_save: 'Сохранить',
		ms_search: 'Поиск',
		ms_reset: 'Сброс',
		ms_send_email: 'Отправить письмо',
		ms_students_found: 'Найдено студентов',
		ms_no_students_found: 'Студенты за этот период не найдены',
		ms_student_id: 'ID студента',
		ms_student_name: 'Имя',
		ms_email: 'Email',
		ms_last_updated: 'Последнее обновление',
		ms_last_updated_by: 'Последний обновил',
		ms_last_activity: 'Последняя активность',
		ms_never_active_title: 'Неактивные студенты',
		ms_never_active_description: 'Поиск студентов, которые ни разу не отправили черновик с момента регистрации.',
		ms_registered_at: 'Дата регистрации',
		ms_never_active_students: 'Неактивные студенты',
		ms_never_active_hint: '(не подали ни одного черновика с момента регистрации)',
		ms_setting_updated: 'Настройка успешно обновлена',
		ms_error_loading: 'Ошибка загрузки настроек',
		ms_error_updating: 'Ошибка обновления настройки',
		ms_error_searching: 'Ошибка поиска студентов',
		ms_error_sending: 'Ошибка отправки писем',
		ms_fill_all_fields: 'Заполните все поля',
		ms_emails_sent: 'Письма отправлены',
		ms_send_result: 'Результат отправки',
		ms_successful: 'успешно',
		ms_failed: 'неудачно',
		ms_year: 'год',
		ms_month: 'мес.',
		ms_week: 'нед.',
		ms_day: 'дн.',
		ms_days: 'дн.',
		ms_schedule_day_of_month: 'День отправки (ежемесячно)',
		ms_schedule_hour: 'Час отправки',
		ms_schedule_day_of_month_hint: 'День месяца для отправки (1–28)',
		ms_schedule_hour_hint: 'Час отправки (0–23)',
		// Sidebar & profile header
		maintenance: 'Техническое обслуживание',
		expected_graduation_month: 'Ожидаемая дата выпуска',
		enrolled_partner_university: 'Партнёрский университет',
		not_set: 'Не задано',
		getLink: 'Получить ссылку',
		generating: 'Генерация...',
		regenerateLink: 'Новая ссылка',
		linkExpiresIn: 'Истекает через',
		link_expired: 'Срок действия истёк',
		link_expiry_notice: 'Срок действия ссылки',
		link_generated_success: 'Ссылка успешно создана!',
		link_generation_failed: 'Не удалось создать ссылку',
		link_copied: 'Ссылка скопирована!',
		linkCopied: 'Ссылка скопирована!',
		profile_not_public: 'Сначала откройте профиль',
		deactivate_link: 'Деактивировать ссылку',
		deactivating: 'Деактивация...',
		link_deactivated_success: 'Ссылка деактивирована',
		link_deactivate_failed: 'Не удалось деактивировать ссылку',
		confirm_to_download_cv: 'Я прочитал / Я прочитала',
		resumeDownloadWarning: `⚠️ Обязательно прочтите перед скачиванием резюме!
Уважаемые студенты!
Это резюме (CV) автоматически генерируется системой на основе информации из вашего портфолио на сайте. Поэтому его содержимое может быть не на 100% точным. После скачивания обязательно сравните его с "見本 履歴書" (Образец резюме) и проверьте данные вручную!
❗️ Частые ошибки, на которые нужно обратить особое внимание:
1) Раздел "職務経歴書" (Опыт работы):
В первую таблицу вносятся только те проекты, в которых вы участвовали через университетскую систему "Cowork". Поскольку данные загружаются автоматически, они могут смешаться с другой информацией. Пожалуйста, строго разделяйте проекты "Cowork" и внешний опыт (подработки, фриланс, проекты других компаний) и проверьте, чтобы всё было записано в соответствующих разделах.

2) Адреса – "現住所" и "連絡先":
"現住所" (Текущий адрес): Здесь указывается адрес, по которому вы проживаете в данный момент.
"連絡先" (Контактный адрес): Здесь указывается адрес проживания ваших опекунов (родителей).
Если оба адреса совпадают, просто напишите "同上" (То же, что и выше) в нижнем разделе (Контактный адрес). Пожалуйста, проверьте правильность заполнения.

3) Раздел "自己PR" (О себе):
Этот раздел нельзя оставлять в виде автоматически сгенерированного шаблона. Вам нужно каждый раз переписывать его самостоятельно, адаптируя под конкретную компанию или ситуацию. Убедитесь, что текст исправлен и написан вашими собственными словами.
💡 Если у вас возникнут вопросы или трудности при редактировании резюме, не стесняйтесь обращаться за помощью к семпаям или сотрудникам (staff) университета.`,
		companies: 'Компании',
		add_recruiter: 'Добавить рекрутера',
		choose_existing_company: 'Выбрать существующую компанию',
		create_new_company: 'Создать новую компанию',
		confirm_delete_recruiter: 'Вы уверены, что хотите удалить этого рекрутера?',
		not_assign_company: 'Вы не можете назначить рекрутера без компании.',
		companies: 'Компании',
		new_company: 'Новая компания',
		company_name: 'Название компании',
		company_name_placeholder: 'Введите название компании, ',
		is_partner_true: 'Партнер',
		is_partner_false: 'Не партнер',
		company_address: 'Адрес',
		recruiters: 'Рекрутеры',
		no_companies_found: 'Компании не найдены',
		is_partner: 'Партнер',
		partner_chip: 'Партнер',
		set_parent_recruiter: 'Назначен главным рекрутером',
		removed_parent_recruiter: 'Удален из главных рекрутеров',
		assign_recruiter: 'Назначить рекрутера',
		create_company: 'Создать компанию',
		company_identity: 'Идентификация компании',
		no_recruiters_found: 'Рекрутеры не найдены',
		search_recruiters: 'Поиск рекрутеров...',
		select_company: 'Выберите компанию',
		assigned_recruiter_to_company: 'Рекрутер успешно назначен компании',
		unassigned_recruiter_from_company: 'Рекрутер успешно удален из компании',
		updated_company_identity: 'Идентификация компании успешно обновлена',
		updated_company_profile: 'Профиль компании успешно обновлен',
		details: 'Детали',
		edit_recruiter: 'Редактировать рекрутера',
		company_change_hint: 'Компания может быть изменена только со страницы компании.',
	},
}

// Alias 'jp' to 'ja' to support users selecting 'jp'
translations.jp = translations.ja

export default translations
